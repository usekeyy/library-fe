...
export const fetchMaterialServiceService = (parameter) => {
    return dispatch => {
        return new Promise((resolve, reject) => {
            axios.get('material_service/service', {
                ...
                .then(response => {
                    resolve(response);
                    dispatch({
                        type: materialServiceTypes.SUCCESS_FETCH_ITEM_MATERIAL_SERVICE_SERVICE,
                        data: response.data
                    })
                })
                .catch(error => {
                    reject(error.response);
                    dispatch({ type: materialServiceTypes.FAIL_FETCH_ITEM_MATERIAL_SERVICE_SERVICE, response: error.response })
                });
        })
    }
}
