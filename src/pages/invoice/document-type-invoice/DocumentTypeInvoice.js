...
import { fetchDocumentTypeInvoice, showDocumentTypeInvoice, deleteDocumentTypeInvoice, saveDocumentTypeInvoice, updateDocumentTypeInvoice, syncDocumentTypeInvoice } from '../../../store/actions/invoice/documentTypeInvoiceActions';
...
class DocumentTypeInvoice extends Component {
    constructor(props) {
        super(props)
    ...
    asyncData = async () => {
        ...
        this.props.fetchDocumentTypeInvoice(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
	...
	handleSync = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.syncDocumentTypeInvoice(val)
			.then(res => {
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.saveDocumentTypeInvoice(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateDocumentTypeInvoice(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteDocumentTypeInvoice(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">{t("documentType:title")} Invoice</li>
                </ol>
                <h1 className="page-header">{t("documentType:title")} Invoice</h1>
                <Panel loading={false}>
                    <PanelHeader>
                        {t("documentType:table-title")} Invoice
                    </PanelHeader>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showDocumentTypeInvoice={this.props.showDocumentTypeInvoice}
...
const dispatchToProps = dispatch => {
    return {
        fetchDocumentTypeInvoice: (params) => dispatch(fetchDocumentTypeInvoice(params)),
        showDocumentTypeInvoice: (id) => dispatch(showDocumentTypeInvoice(id)),
        deleteDocumentTypeInvoice: (id) => dispatch(deleteDocumentTypeInvoice(id)),
        saveDocumentTypeInvoice: (id, payload) => dispatch(saveDocumentTypeInvoice(id, payload)),
        updateDocumentTypeInvoice: (id, payload) => dispatch(updateDocumentTypeInvoice(id, payload)),
		syncDocumentTypeInvoice: (payload) => dispatch(syncDocumentTypeInvoice(payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(DocumentTypeInvoice));
