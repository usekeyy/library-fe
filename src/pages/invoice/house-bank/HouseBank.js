...
import { fetchHouseBank, showHouseBank, deleteHouseBank, saveHouseBank, updateHouseBank, syncHouseBank } from '../../../store/actions/invoice/houseBankActions';
...
class HouseBank extends Component {
    constructor(props) {
        super(props)
        ...
        this.columns = [
            {
                ...
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="house_bank" onChange={(event) => this.onChanged(event)} value={this.state.params.house_bank} />
                )
            },
            {
    ...
    asyncData = async () => {
        ...
        this.props.fetchHouseBank(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.saveHouseBank(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateHouseBank(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteHouseBank(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">House Bank</li>
                </ol>
                <h1 className="page-header">House Bank</h1>
                <Panel loading={false}>
                    <PanelHeader>
                        Table House Bank
                    </PanelHeader>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showHouseBank={this.props.showHouseBank}
...
const dispatchToProps = dispatch => {
    return {
        fetchHouseBank: (params) => dispatch(fetchHouseBank(params)),
        showHouseBank: (id) => dispatch(showHouseBank(id)),
        deleteHouseBank: (id) => dispatch(deleteHouseBank(id)),
        saveHouseBank: (id, payload) => dispatch(saveHouseBank(id, payload)),
        updateHouseBank: (id, payload) => dispatch(updateHouseBank(id, payload)),
		syncHouseBank: (payload) => dispatch(syncHouseBank(payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(HouseBank));
