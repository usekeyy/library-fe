...
import { fetchUnbilled, showUnbilled, fetchServiceLine } from '../../../../store/actions/invoice/unbilledActions';
    ...
    super(props);
		...
		this.state = {
			...
			param_invoice: {
				...
				ppn: [],
				with_ppn: 'n',
				with_ppn_param: {value: 'n', label: 'Tidak'},
  ...
  componentDidMount = () => {
	...
	submitUnbilled = () => {
		if (this.state.data_invoice.items.length === 0) {
			toastr.error("Mohon pilih item Jasa terlebih dahulu")
			return
		}
		...
		items.forEach((element, i) => {
			...
			param_invoice.total = param_invoice.amount
		...
		param_invoice.amount_temp = param_invoice.amount
        ...
        this.props.fetchServiceLine({goods_receipt_item_id: arr_item.join(';')})
			.then((resp) => {
				let datas = resp.data.data
				let arr_service_line = []
				datas.forEach(element => {
					if (element.service_line.length > 0) {
						element.service_line.forEach(sl => {
							sl.add_cost_id = null
							sl.acc_assignment_category_id = element.acc_assignment_category_id
							sl.acc_assignment_category_number = element.acc_assignment_category_number
							sl.gl_account = element.gl_account
							sl.goods_receipt_number = element.goods_receipt_number
							sl.goods_receipt_item_id = element.goods_receipt_item_id
							sl.service_line_id = sl.id
							sl.add_cost_id = null
							sl.with_ppn_param = []
							sl.with_ppn = 'n'
							sl.net_value = sl.amount_value
							sl.company_id = element.company_id
							sl.qty_awal = sl.qty
							arr_service_line.push(sl)
						});
					}
				});
				param_invoice.items = arr_service_line

				if (arr_penalty.length > 0) {
					this.props.fetchPenalty({array_goods_receipt_item_id: arr_penalty, status: 'd'})
						.then((resp) => {
							let data_penalty = resp.data.data
							param_invoice.penalty = data_penalty
							data_penalty.forEach(element => {
								param_invoice.potongan += parseFloat(element.amount)
							});
							param_invoice.potongan = parseFloat(param_invoice.potongan).toFixed(2)
							param_invoice.total = (parseFloat(param_invoice.total) - parseFloat(param_invoice.potongan)).toFixed(2)
							param_invoice.total_temp = param_invoice.total
			...
			.catch((resp) => {
				console.log(resp)
				toastr.error('Get Data Service Line Error!' + resp.data.message);
				return
			});
	}
	...
	setOption = async (payload, code, key) => {
        ...
        switch(code) {
            case 'qty':
                ...
                data.amount = (parseFloat(data.amount) - parseFloat(data.items[key].amount_value)).toFixed(2)

                data.items[key].amount_value = (parseFloat(new_value) * parseFloat(data.items[key].unit_price)).toFixed(2)
                data.items[key].net_value = data.items[key].amount_value
                data.amount = (parseFloat(data.amount) + parseFloat(data.items[key].amount_value)).toFixed(2)
                data.total = (parseFloat(data.total) + parseFloat(data.amount)).toFixed(2)
    ...
    resetScanFakturPajak = (e) => {
		...
		// 	// amount += parseFloat(element.amount_value)
		// 	if (element.with_ppn === 'y') {
		// 		ppn += parseFloat(element.amount_value)/10
		// 	}
		// });
		// param_invoice.amount = amount
	...
	setWithPPN = (payload) => {
		...
		if (payload === 'y') {
			// param_invoice.ppn_amount = (parseFloat(param_invoice.amount)/10)
			// param_invoice.total = parseFloat(param_invoice.amount) + parseFloat(param_invoice.ppn_amount) - parseFloat(param_invoice.potongan)
			param_invoice.items.forEach(element => {
				...
				{this.state.statusForm === false &&
					<div>
						<ol className="breadcrumb float-xl-right">
							...
							<li className="breadcrumb-item active">Unbilled - Jasa</li>
						</ol>
						<h1 className="page-header">Unbilled - Jasa </h1>
						<div className="row">
...
const dispatchToProps = dispatch => {
	return {
		...
		fetchServiceLine: (params) => dispatch(fetchServiceLine(params)),
		fetchMultipleDetailPurchaseOrder: (id) => dispatch(fetchMultipleDetailPurchaseOrder(id)),
...
export default connect(stateToProps, dispatchToProps)( withTranslation() (Jasa));