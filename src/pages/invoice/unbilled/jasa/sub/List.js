...
class List extends Component {
    ...
    super(props);
		...
		this.columns = [
			...
			},
			{
				Header:'Gross Price',
				id: "gross_price",
                accessor: d => d.gross_price,
				Filter: ({ filter, onChange }) => (
					<input type="text" className="form-control" name="gross_price" onChange={(event) => this.handleFilterNilai(event)} value={this.state.options.gross_price} />
                ),
                Cell: ({ value }) => (
    ...
    edits(e, value) {
		...
		switch (data[index].category) {
			case 'Jasa':
				const url = '/invoice/unbilled/jasa/detail/' + value
	...
	render(){
		return (
			<div>
				<Panel>
					<PanelHeader>List Unbilled - Jasa</PanelHeader>
					<PanelBody>
						<Row>
							<Col sm="12">
