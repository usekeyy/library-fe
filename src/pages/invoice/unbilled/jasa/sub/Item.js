	...
	render(){
		...
		if (items.length > 0) {
			...
			rows = items.map((dt, i) => {
				...
				return (
					<tr key={i}>
						...
						<td>{dt.uom}</td>
						<td align="right">{formatNumber2(dt.gross_price, 2)}</td>
			...
			if(loadings.showItems){
				rows = (<RowEmpty colSpan='18'><i className="fas fa-spinner fa-pulse"></i>Loading</RowEmpty>);
			} else {
				rows = (<RowEmpty colSpan='18'>Tidak ada data</RowEmpty>);
			}
		}
