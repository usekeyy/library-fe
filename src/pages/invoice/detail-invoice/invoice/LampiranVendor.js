...
const Penalty = (props) => {
    ...
    let rows;
    if (props.data_lampiran_vendor !== undefined) {
        rows = Object.keys(props.data_lampiran_vendor).map(function (key, index) {
            return (
                <tr key={key}>
                    <td>{index+1}</td>
                    <td>{formatDate(props.data_lampiran_vendor[key]['document_date'], false)}</td>
                    <td>{props.data_lampiran_vendor[key]['description']}</td>
                    <td>
                        {props.data_lampiran_vendor[key]['id'] === undefined ?
                        <a className="col-form-label" onClick={(e) => props.toggleOpenPreview(e, props.data_lampiran_vendor[key]['file'], `${process.env.REACT_APP_API_BASE_URL}files/temp/${props.data_lampiran_vendor[key]['file']}`)} href="/">{props.data_lampiran_vendor[key]['file']}</a>
                        :
                        <a className="col-form-label" onClick={(e) => props.toggleOpenPreview(e, props.data_lampiran_vendor[key]['file'], `${process.env.REACT_APP_API_BASE_URL}files/invoice/${props.data_lampiran_vendor[key]['file']}`)} href="/">{props.data_lampiran_vendor[key]['file']}</a>
                        }
                    </td>
                    <td>
                        {(props.data.status === 'draft' || props.data.status === 'rejected_1' || props.data.status === 'rejected_2') && props.user.has_roles.includes("VNDR01") &&
                            <button className="btn btn-xs btn-danger" onClick={(e) => props.toggleDelete(e, key, 'lampiran-vendor')} ><i className="fa fa-trash"></i></button>
                        }
                    </td>
                </tr>
            )
        ...
        <div>
            <Panel className="margin-bot-false">
                ...
                {props.user.uuid === props.data.created_by && (props.data.status === 'draft' || props.data.status === 'rejected_1' || props.data.status === 'rejected_2') &&
                    <PanelBody >
                        {props.loadings.input_lampiran_vendor && (
                            <center>
                            <br />
                        ...
                        <div className="row">
                            {props.loadings.input_lampiran_vendor === false && (
                ...
                <PanelBody >
                    {props.loadings.list_lampiran_vendor && (
                        <center>
                        <br />
                        <ReactLoading type="cylon" color="#0f9e3e" />
                        <br />
                    ...
                    {props.loadings.list_lampiran_vendor === false && (
                        <div className="row">
                            <div className="col-sm-12">
