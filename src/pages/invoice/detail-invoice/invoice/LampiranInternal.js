        ...
        rows = Object.keys(props.data_lampiran_internal).map(function (key, index) {
            return (
                <tr key={key}>
                    ...
                    <td>
                        {props.data_lampiran_internal[key]['id'] === undefined &&
                            <button className="btn btn-xs btn-danger" onClick={(e) => props.toggleDelete(e, key, 'lampiran-internal')} ><i className="fa fa-trash"></i></button>
                        }
                    </td>
                </tr>
            )
    ...
    return (
        <div>
            <Panel className="margin-bot-false">
                ...
                {(((props.data.status === 'submitted' || props.data.status === 'rejected_hc') && props.user.has_roles.includes("INVER1")) || ((props.data.status === 'sent' || props.data.status === 'received' || props.data.status === 'posted' || props.data.status === 'rejected_hc_bendahara') && props.user.has_roles.includes("INVER2"))) &&
                    <PanelBody >
