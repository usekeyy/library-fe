...
import { fetchReferenceKey, showReferenceKey, deleteReferenceKey, saveReferenceKey, updateReferenceKey, syncReferenceKey } from '../../../store/actions/invoice/referenceKeyActions';
...
class ReferenceKey extends Component {
    constructor(props) {
        super(props)
        ...
        this.columns = [
            ...
            {
				Header: "NPWP Address",
                accessor: "npwp_address",
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="npwp_address" onChange={(event) => this.onChanged(event)} value={this.state.params.npwp_address} />
                )
            },
            {
				Header: "NPWP Address",
                accessor: "npwp_number",
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="npwp_number" onChange={(event) => this.onChanged(event)} value={this.state.params.npwp_number} />
                )
            },
            {
				Header: "Company ID",
    ...
    asyncData = async () => {
        ...
        this.props.fetchReferenceKey(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
	...
	handleSync = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.syncReferenceKey(val)
			.then(res => {
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.saveReferenceKey(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateReferenceKey(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteReferenceKey(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">Payment Block</li>
                </ol>
                <h1 className="page-header">Payment Block</h1>
                <Panel loading={false}>
                    <PanelHeader>
                        Table Payment Block
                    </PanelHeader>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showReferenceKey={this.props.showReferenceKey}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showReferenceKey: (id) => dispatch(showReferenceKey(id)),
        deleteReferenceKey: (id) => dispatch(deleteReferenceKey(id)),
        saveReferenceKey: (id, payload) => dispatch(saveReferenceKey(id, payload)),
        updateReferenceKey: (id, payload) => dispatch(updateReferenceKey(id, payload)),
		syncReferenceKey: (payload) => dispatch(syncReferenceKey(payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(ReferenceKey));
