...
import { fetchPaymentMethod, showPaymentMethod, deletePaymentMethod, savePaymentMethod, updatePaymentMethod, syncPaymentMethod } from '../../../store/actions/invoice/paymentMethodActions';
...
class PaymentMethod extends Component {
        ...
        this.columns = [
            {
                ...
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="payment_method" onChange={(event) => this.onChanged(event)} value={this.state.params.payment_method} />
                )
            },
            {
    ...
    asyncData = async () => {
        ...
        this.props.fetchPaymentMethod(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
	...
	handleSync = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.syncPaymentMethod(val)
			.then(res => {
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.savePaymentMethod(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updatePaymentMethod(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deletePaymentMethod(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">Payment Method</li>
                </ol>
                <h1 className="page-header">Payment Method</h1>
                <Panel loading={false}>
                    <PanelHeader>
...
const dispatchToProps = dispatch => {
    return {
        ...
        showPaymentMethod: (id) => dispatch(showPaymentMethod(id)),
        deletePaymentMethod: (id) => dispatch(deletePaymentMethod(id)),
        savePaymentMethod: (id, payload) => dispatch(savePaymentMethod(id, payload)),
        updatePaymentMethod: (id, payload) => dispatch(updatePaymentMethod(id, payload)),
		syncPaymentMethod: (payload) => dispatch(syncPaymentMethod(payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(PaymentMethod));
