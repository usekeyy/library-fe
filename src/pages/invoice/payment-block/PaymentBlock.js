...
import { fetchPaymentBlock, showPaymentBlock, deletePaymentBlock, savePaymentBlock, updatePaymentBlock, syncPaymentBlock } from '../../../store/actions/invoice/paymentBlockActions';
...
class PaymentBlock extends Component {
    ...
    asyncData = async () => {
        ...
        this.props.fetchPaymentBlock(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
	...
	handleSync = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.syncPaymentBlock(val)
			.then(res => {
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.savePaymentBlock(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updatePaymentBlock(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deletePaymentBlock(type)
            .then((resp) => {
            ...
            <div>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showPaymentBlock={this.props.showPaymentBlock}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showPaymentBlock: (id) => dispatch(showPaymentBlock(id)),
        deletePaymentBlock: (id) => dispatch(deletePaymentBlock(id)),
        savePaymentBlock: (id, payload) => dispatch(savePaymentBlock(id, payload)),
        updatePaymentBlock: (id, payload) => dispatch(updatePaymentBlock(id, payload)),
		syncPaymentBlock: (payload) => dispatch(syncPaymentBlock(payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(PaymentBlock));
