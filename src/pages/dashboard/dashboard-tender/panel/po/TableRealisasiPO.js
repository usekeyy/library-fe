...
import { getTenderSummaryPOAccGroup, downloadExcelRealisasiPO } from '../../../../../store/actions/dashboard/dashboardTenderAction';
// import { downloadExcelPerformanceReport } from '../../../../../store/actions/vendor/performanceReportActions';
import { formatNumber } from '../../../../../helpers/formatNumber';

class TableRealisasiPO extends Component {
    constructor(props) {
        super(props)
        ...
        this.columns = [
            ...
            },
            {
                ...
                id: "acct_group",
                accessor: d => d.acct_group,
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="acct_group" onChange={(event) => this.onChanged(event)} value={this.state.params.acct_group} />
                )
            },
            {
                Header: "Account Group Description",
                id: "acct_group_deskripsi",
                accessor: d => d.acct_group_deskripsi,
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="acct_group_deskripsi" onChange={(event) => this.onChanged(event)} value={this.state.params.acct_group_deskripsi} />
                )
            },
            {
                Header: "Jumlah PO",
            ...
            },
            {
                ...
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" placeholder="Without Coma (,) or Dot (.)" name="total_nilai" onChange={(event) => this.onChanged(event)} value={this.state.params.total_nilai} />
                ),
                Cell : ({original}) => (
                    <div style={{textAlign:"right"}}>
                       {formatNumber(original.total_nilai,2)}
                    </div>                    
                )
            },
            {
...

            {
                ...
                Cell: d => (
                    <React.Fragment>
                        <center>
                        <button className="btn btn-xs btn-primary" type="button" onClick={(e) => this.toDetail(e, d.original.source, d.original.purch_org_id, d.original.acct_group)}>Detail</button>
                        </center>
                    </React.Fragment>
                )
            },
    ...
    asyncData = async () => {
        ...
        this.props.getTenderSummaryPOAccGroup(this.state.params)
            .then((resp) => {
                this.setState({
                    // data: resp.data.data,
                    data: resp.data.data,
    ...
    toDetail(e,source, purchasing_org_id, purchasing_group_id) {
        this.props.props.history.push('/dashboard/tender/detail/report-of-po-acc/'+source+'/'+purchasing_org_id+'/'+purchasing_group_id)
    }
    ...
    downloadExcel = () => {
        ...
        this.props.downloadExcelRealisasiPO()
            .then(res => {
        ...
        return (
            <div>
                <Panel collapse={navigator.userAgent.indexOf("Firefox") > -1 ? false : true}>
                    <PanelHeader>
                        Report Realisasi PO Per Account Group (Sinergi BUMN & PI Group)
                    </PanelHeader>
                    <PanelBody>
                        <Col sm="12">
...
const dispatchToProps = dispatch => {
    return {
        getTenderSummaryPOAccGroup: (params) => dispatch(getTenderSummaryPOAccGroup(params)),
        downloadExcelRealisasiPO: () => dispatch(downloadExcelRealisasiPO()),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(TableRealisasiPO));
