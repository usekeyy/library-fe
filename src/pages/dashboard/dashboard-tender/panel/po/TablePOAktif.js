...
import { getTenderPOAktifTable, downloadExcelPOAktif } from '../../../../../store/actions/dashboard/dashboardTenderAction';
...
class TablePOAktif extends Component {
    constructor(props) {
        super(props)
        ...
        this.columns = [
			...
			},
            {
                ...
                Cell: ({value}) => (
                    <React.Fragment>
                        <center>
                            <h5><Badge color={value === 'open' ?  "success" : "warning"}>{value}</Badge></h5>
                        </center>
                    </React.Fragment>
                )
			},
    ...
    asyncData = async () => {
        ...
        this.props.getTenderPOAktifTable(this.state.params)
            .then((resp) => {
    ...
    downloadExcel = () => {
        ...
        this.props.downloadExcelPOAktif()
			.then(res => {
                const url = window.URL.createObjectURL(new Blob([res.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'Report_PO_Aktif.xlsx'); //or any other extension
...
const dispatchToProps = dispatch => {
	return {
		...
		getTenderPOAktifTable: (params) => dispatch(getTenderPOAktifTable(params)),
        downloadExcelPOAktif : () => dispatch(downloadExcelPOAktif()),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (TablePOAktif));
