...
import { getVendorStatusTable, downloadExcelReportSOS } from '../../../../../store/actions/dashboard/dashboardVendorAction';
// import {downloadExcelPerformanceReport} from '../../../../../store/actions/vendor/performanceReportActions';

class TableSos extends Component {
    constructor(props) {
        super(props)
        ...
        this.columns = [
			...
			},
            {
                Header: "SoS Header",
				id: "bidang_usaha",
				accessor: d => d.bidang_usaha,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="bidang_usaha" onChange={(event) => this.onChanged(event)} value={this.state.params.bidang_usaha} />
                ),
                Cell : ({value}) => (
                    <div>
                        {this.setSoSHeader(value)} 
                    </div>
...

            {
                Header: "SoS Item",
				id: "sub_bidang_usaha",
				accessor: d => d.sub_bidang_usaha,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="sub_bidang_usaha" onChange={(event) => this.onChanged(event)} value={this.state.params.sub_bidang_usaha} />
                ),
                Cell : ({value}) => (
                    <div>
                        {this.setSoSHeader(value)} 
			...
			},
            {
				...
				accessor: d => d.merk,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="merk" onChange={(event) => this.onChanged(event)} value={this.state.params.merk} />
                ),
                Cell : ({value}) => (
                    <div>
                        {this.setSoSHeader(value)} 
    ...
    setSoSHeader(value){
        if (value){
            const temp = value.split('; ')?.map((item, index) => {
                return (
    ...
    asyncData = async () => {
        ...
        this.props.getVendorStatusTable(this.state.params)
            .then((resp) => {
    ...
    getCheck = (check) => {
        ...
        filters.start = 0;
        if (check.length > 0) {
            filters.status_name = check.join(";");
    ...
    downloadExcel = () => {
        ...
        this.props.downloadExcelReportSOS()
			.then(res => {
                const url = window.URL.createObjectURL(new Blob([res.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'Report_Vendor_By_Purchasing_Organization,Status,SoS.xlsx'); //or any other extension
...
const dispatchToProps = dispatch => {
	return {
		...
		getVendorStatusTable: (params) => dispatch(getVendorStatusTable(params)),
        downloadExcelReportSOS : () => dispatch(downloadExcelReportSOS()),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (TableSos));
