...
import { getPaymentLeadTimeTable, downloadExcelPaymentLeadTime } from '../../../../store/actions/dashboard/dashboardInvoiceAction';
// import FilterDate from '../../../../components/filterdate/FilterDate';
import { formatDate } from '../../../../helpers/formatDate';
import { formatNumber } from '../../../../helpers/formatNumber';

class TableChartInvoicePaymentLead extends Component {
    constructor(props) {
        super(props)
        ...
        this.columns = [
            ...
            {
                Header: "Verifikator 1 Approved",
				id: "verif1_approve",
				accessor: d => formatDate(d.verif1_approve,2),
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="verif1_approve" onChange={(event) => this.onChanged(event)} value={this.state.params.verif1_approve} />
                )
			},
            {
                Header: "Verifikator 2 Approved",
				id: "verif2_approve",
				accessor: d => formatDate(d.verif2_approve,2),
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="verif2_approve" onChange={(event) => this.onChanged(event)} value={this.state.params.verif2_approve} />
                )
			},
            {
                Header: "Paid",
				id: "paid",
				accessor: d => formatDate(d.paid,2),
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="paid" onChange={(event) => this.onChanged(event)} value={this.state.params.paid} />
                )
			},
            {
                Header: "Total (Days)",
    ...
    asyncData = async () => {
        ...
        this.props.getPaymentLeadTimeTable(this.state.params)
            .then((resp) => {
    ...
    downloadExcel = () => {
        ...
        this.props.downloadExcelPaymentLeadTime()
			.then(res => {
                const url = window.URL.createObjectURL(new Blob([res.data]));
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', 'Invoice_Payment_Lead_Time.xlsx'); //or any other extension
        ...
        return (
            <div>
                <Panel collapse={navigator.userAgent.indexOf("Firefox") > -1 ? false : true}>
                    <PanelHeader>
                        Report Invoice - Payment Lead Time
                    </PanelHeader>
                    <PanelBody>
                            <Col sm="12">
...
const dispatchToProps = dispatch => {
	return {
		...
		getPaymentLeadTimeTable: (params) => dispatch(getPaymentLeadTimeTable(params)),
        downloadExcelPaymentLeadTime : () => dispatch(downloadExcelPaymentLeadTime()),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (TableChartInvoicePaymentLead));
