...
import {fetchPlant, savePlant, showPlant, updatePlant, deletePlant} from '../../../store/actions/master/plantActions';
...
class Plant extends Component {
    ...
    super(props);
		...
		this.columns = [
			{
				Header: () => this.props.t("plant:label.id"),
				id: "id",
				className: "sticky",
			...
			{
				Header: () => this.props.t("plant:label.name"),
				id: "name",
				accessor: d => d.name,
				Filter: ({ filter, onChange }) => (
			...
			{
				Header: () => this.props.t("plant:label.district-code"),
				id: "postal_code",
			...
			{
				Header: () => this.props.t("plant:label.district"),
				id: "district_name",
			...
			{
				Header: () => this.props.t("plant:label.purchasing-org-code"),
			...
			{
				Header: () => this.props.t("plant:label.purchasing-org"),
			...
			{
				Header: () => this.props.t("plant:label.status"),
				id: "status",
			...
			{
				Header: () => this.props.t("plant:label.created_at"),
				id: "created_at",
			...
			{
				Header: () => this.props.t("plant:label.updated_at"),
				id: "updated_at",
			...
			{
				Header: () => this.props.t("plant:label.action"),
				id:"action",
				accessor: "uuid",
				filterable: false,
				sortable: false,
  ...
  componentDidMount = () => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updatePlant(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">{t("plant:title")}</li>
				</ol>
				<h1 className="page-header">{t("plant:title")} </h1>
				<Panel loading={false}>
					<PanelHeader>{t("plant:table-title")}</PanelHeader>
						...
						<Row>
							...
							<Col sm="6">
								<div className="pull-right m-b-10">
									{this.props.access.C && <Button color="primary" size="sm" value="" onClick={(e) => this.toggleFormOpen(e)} >{t("plant:button.add")}</Button>}
								</div>
							</Col>
						</Row>
...
const dispatchToProps = dispatch => {
	return {
		...
		savePlant: (payload) => dispatch(savePlant(payload)),
		showPlant: (id) => dispatch(showPlant(id)),
		updatePlant: (id, payload) => dispatch(updatePlant(id, payload)),
...
export default connect(stateToProps, dispatchToProps)( withTranslation() (Plant));