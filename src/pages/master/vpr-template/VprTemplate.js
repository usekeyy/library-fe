...
import { fetchVprTemplate, showVprTemplate, deleteVprTemplate, saveVprTemplate, updateVprTemplate, fetchVprCategory, fetchVprSubCategory } from '../../../store/actions/master/vprActions';
...
class VprTemplate extends Component {
    constructor(props) {
        super(props)
        ...
        this.columns = [
            {
				Header: () => this.props.t("documentType:label.id"),
                accessor: "id",
                height: 10,
            ...
            {
				Header: () => this.props.t("documentType:label.name"),
                accessor: "name",
                Filter: ({ filter, onChange }) => (
                ...
                Cell: ({ value }) => (
                    <React.Fragment>
                        <center>
                            ...
                            {this.props.access.U && <button className="btn btn-xs btn-warning" value={value} onClick={(e) => this.toggleFormOpen(e, value)} ><i className="fa fa-edit"></i></button>}
                            {this.props.access.D && <button className="btn btn-xs btn-danger" value={value} onClick={(e) => this.toggleConfirm(e, value)} ><i className="fa fa-trash"></i></button>}
                            {this.props.access.R && <button className="btn btn-xs btn-success" value={value} onClick={() => this.toDetail(value)} ><i className="fa fa-eye"></i></button>}
                        </center>
                    </React.Fragment>
                )
            },
    ...
    asyncData = async () => {
        ...
        this.props.fetchVprTemplate(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    fetchCategory = async () => {
        ...
        this.props.fetchVprCategory()
            .then((resp) => {
                ...
                let options = category.map((dt) => {
                    return { value: dt.id, label: dt.category };
                })
                this.setState({categoryData : options})
    ...
    fetchSubCategory = async () => {
        this.setState(({ loadings }) => ({ loadings: { ...loadings, subCategory: true }}));
        this.props.fetchVprSubCategory()
            .then((resp) => {
                console.log(resp.data.data)
                let subCategory = resp.data.data;
                let options = subCategory.map((dt) => {
                    return { value: dt.id, label: dt.category_name + " - " + dt.description, category_id : dt.vpr_category_id };
                })
                this.setState({subCategoryData : options})

                this.setState(({ loadings }) => ({ loadings: { ...loadings, subCategory: false }}));
            })
            .catch((resp) => {
                this.setState(({ loadings }) => ({ loadings: { ...loadings, subCategory: false }}));
                toastr.error(resp.data.status, resp.data.message);
    ...
    toDetail = (uuid) => {
        this.props.history.push('/general/vpr/vpr-template/' + uuid)
    }
    ...
    savePayload = (type, payload) => {
        ...
        if (type === "save") {
            ...
            this.props.saveVprTemplate(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        ...
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateVprTemplate(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteVprTemplate(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">VPR Template</li>
                </ol>
                <h1 className="page-header">VPR Template  <small>Master Data VPR Template</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        VPR Template
                    </PanelHeader>
                        ...
                        <Row>
                            ...
                            <Col sm="6">
                                <div className="pull-right m-b-10">
                                    {/* {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("documentType:button.add")}</Button>} */}
                                    {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("documentType:button.add")}</Button>}
                                </div>
                                {/* <span> </span>
                                <div className="pull-right m-b-10">
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showVprTemplate={this.props.showVprTemplate}
                        ...
                        fetchSubCategory={this.fetchSubCategory}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showVprTemplate: (id) => dispatch(showVprTemplate(id)),
        deleteVprTemplate: (id) => dispatch(deleteVprTemplate(id)),
        saveVprTemplate: (id, payload) => dispatch(saveVprTemplate(id, payload)),
        updateVprTemplate: (id, payload) => dispatch(updateVprTemplate(id, payload)),

        fetchVprCategory: (params) => dispatch(fetchVprCategory(params)),
        fetchVprSubCategory: (params) => dispatch(fetchVprSubCategory(params)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(VprTemplate));
