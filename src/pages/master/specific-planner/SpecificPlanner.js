...
import {fetchSpecificPlanner,
				showSpecificPlanner,
				deleteSpecificPlanner,
				saveSpecificPlanner,
				updateSpecificPlanner} from '../../../store/actions/master/specificPlannerActions';
...
class SpecificPlanner extends Component {
	constructor(props) {
    super(props);
		...
		this.columns = [
			{
				...
				id: "general_planner_name",
				accessor: d => d.general_planner_name,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="general_planner_name" onChange={(event) => this.handleFilter(event)} value={this.state.options.general_planner_name} />
				)
			},
			{
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
      ...
      this.props.fetchSpecificPlanner(params)
      .then((resp) => {
				if(this._isMounted){
	...
	handleSave = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.saveSpecificPlanner(val)
			.then(res => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updateSpecificPlanner(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
	...
	handleDelete = (id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.deleteSpecificPlanner(id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
...
handleFilter = (event) => {
	...
	fetchGeneralPlanner = () => {
		if(this._isMounted){
			this.setState(({ loadings }) => ({ loadings: { ...loadings, general_planner: true }, m_general_planner: [] }));
			this.props.fetchGeneralPlanner()
			.then((resp) => {
				let m_general_planner = resp.data.data;
        let options = m_general_planner.map((dt) => {
          return { value: dt.id, label: `${dt.name}` };
			...
			.catch((resp) => {
				...
				this.setState(({ loadings }) => ({ loadings: { ...loadings, general_planner: false } }));
			});
		}
	}
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">{"Specific Planner"}</li>
				</ol>
				<h1 className="page-header">Specific Planner </h1>
				<Panel loading={false}>
					<PanelHeader>{"Table Specific Planner"}</PanelHeader>
...
onSortedChange={val => {this.changeSorted(val) }}
				...
				showSpecificPlanner={this.props.showSpecificPlanner} 
...
const dispatchToProps = dispatch => {
	return {
		...
		saveSpecificPlanner: (payload) => dispatch(saveSpecificPlanner(payload)),
		showSpecificPlanner: (id) => dispatch(showSpecificPlanner(id)),
		updateSpecificPlanner: (id, payload) => dispatch(updateSpecificPlanner(id, payload)),
		deleteSpecificPlanner: (id) => dispatch(deleteSpecificPlanner(id)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (SpecificPlanner));