...
import { fetchWorkUnits, showWorkUnits, deleteWorkUnits, saveWorkUnits, updateWorkUnits } from '../../../store/actions/master/workUnitsActions';
...
class WorkUnits extends Component {
    constructor(props) {
        ...
        this.columns = [
            {
				Header: () => this.props.t("workUnit:label.id"),
                accessor: "id",
                height: 10,
            ...
            {
				Header: () => this.props.t("workUnit:label.name"),
                accessor: "name",
                Filter: ({ filter, onChange }) => (
            ...
            {
				Header: () => this.props.t("workUnit:label.description"),
            ...
            {
				Header: () => this.props.t("workUnit:label.status"),
                id: "status",
                accessor: d => d.status,
                Filter: ({ filter, onChange }) => (
            ...
            {
				Header: () => this.props.t("workUnit:label.created_at"),
                id: "created_at",
            ...
            {
				Header: () => this.props.t("workUnit:label.updated_at"),
                id: "updated_at",
            ...
            {
				Header: () => this.props.t("workUnit:label.action"),
                filterable: false,
                sortable: false,
                accessor: "uuid",
                Cell: ({ value }) => (
    ...
    asyncData = async () => {
        ...
        this.props.fetchWorkUnits(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.saveWorkUnits(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateWorkUnits(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteWorkUnits(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">{t("workUnit:title")}</li>
                </ol>
                <h1 className="page-header">{t("workUnit:title")}  <small>Master Data {t("workUnit:title")}</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        {t("workUnit:table-title")}
                    </PanelHeader>
                        ...
                        <Row>
                            ...
                            <Col sm="6">
                                <div className="pull-right m-b-10">
                                    {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("workUnit:button.add")}</Button>}
                                </div>
                            </Col>
                        </Row>
                        <Row>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showWorkUnits={this.props.showWorkUnits}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showWorkUnits: (id) => dispatch(showWorkUnits(id)),
        deleteWorkUnits: (id) => dispatch(deleteWorkUnits(id)),
        saveWorkUnits: (id, payload) => dispatch(saveWorkUnits(id, payload)),
        updateWorkUnits: (id, payload) => dispatch(updateWorkUnits(id, payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(WorkUnits));
