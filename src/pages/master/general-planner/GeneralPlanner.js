...
import {fetchGeneralPlanner,
				showGeneralPlanner,
				deleteGeneralPlanner,
				saveGeneralPlanner,
				updateGeneralPlanner} from '../../../store/actions/master/generalPlannerActions';
...
class GeneralPlanner extends Component {
	constructor(props) {
    super(props);
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
      ...
      this.props.fetchGeneralPlanner(params)
      .then((resp) => {
				if(this._isMounted){
	...
	handleSave = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.saveGeneralPlanner(val)
			.then(res => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updateGeneralPlanner(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
	...
	handleDelete = (id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.deleteGeneralPlanner(id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
...
handleFilter = (event) => {
	...
	fetchCompanies = () => {
		if(this._isMounted){
			this.setState(({ loadings }) => ({ loadings: { ...loadings, company: true }, m_company: [] }));
        ...
        let options = m_company.map((dt) => {
          return { value: dt.id, label: dt.id+' - '+dt.name };
			...
			.catch((resp) => {
				...
				this.setState(({ loadings }) => ({ loadings: { ...loadings, company: false } }));
			});
		}
	}
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">{"General Planner"}</li>
				</ol>
				<h1 className="page-header">General Planner </h1>
				<Panel loading={false}>
					<PanelHeader>{"Table General Planner"}</PanelHeader>
...
onSortedChange={val => {this.changeSorted(val) }}
				...
				showGeneralPlanner={this.props.showGeneralPlanner} 
...
const dispatchToProps = dispatch => {
	return {
		...
		saveGeneralPlanner: (payload) => dispatch(saveGeneralPlanner(payload)),
		showGeneralPlanner: (id) => dispatch(showGeneralPlanner(id)),
		updateGeneralPlanner: (id, payload) => dispatch(updateGeneralPlanner(id, payload)),
		deleteGeneralPlanner: (id) => dispatch(deleteGeneralPlanner(id)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (GeneralPlanner));