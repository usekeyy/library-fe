...
import { fetchTipeRekanan, saveTipeRekanan , showTipeRekanan , updateTipeRekanan , deleteTipeRekanan } from '../../../store/actions/master/tipeRekananActions';
...
class VendorTipeRekanan extends Component {
    constructor(props) {
        super(props)
    ...
    asyncData = async () => {
        ...
        this.props.fetchTipeRekanan(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (payload) => {
        if (this._isMounted) {
            ...
            this.props.saveTipeRekanan(payload)
                .then((resp) => {
    ...
    updatePayload = (id, payload) => {
        if (this._isMounted) {
            this.setState({ loadingSubmit: true });
            this.props.updateTipeRekanan(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteTipeRekanan(type)
            .then((resp) => {
                toastr.success(resp.data.message);
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active"> {t("vendorTipeRekanan:title")} </li>
                </ol>
                <h1 className="page-header">{t("vendorTipeRekanan:title")} <small>Master {t("vendorTipeRekanan:title")}</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                    {t("vendorTipeRekanan:title")}
                    </PanelHeader>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        fetchTipeRekanan={this.props.fetchTipeRekanan}
                        showTipeRekanan={this.props.showTipeRekanan}
...
const dispatchToProps = dispatch => {
    return {
        ...
        saveTipeRekanan: (id) => dispatch(saveTipeRekanan(id)),
        showTipeRekanan: (id) => dispatch(showTipeRekanan(id)),
        updateTipeRekanan: (id, payload) => dispatch(updateTipeRekanan(id, payload)),
        deleteTipeRekanan: (id, payload) => dispatch(deleteTipeRekanan(id, payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(VendorTipeRekanan));
