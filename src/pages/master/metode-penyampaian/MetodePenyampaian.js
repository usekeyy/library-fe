...
import { fetchMetodePenyampaian, saveMetodePenyampaian, updateMetodePenyampaian, showMetodePenyampaian, deleteMetodePenyampaian } from '../../../store/actions/master/metodePenyampaianActions';
...
class MetodePenyampaian extends Component {
    constructor(props) {
        super(props)
    ...
    asyncData = async () => {
        ...
        this.props.fetchMetodePenyampaian(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (payload) => {
        if(this._isMounted){
            ...
            this.props.saveMetodePenyampaian(payload)
            .then((resp) => {
                toastr.success(resp.data.message);
                this.setState({ modalOpen: false ,errors:[], loadingSubmit:false}, () => this.req())
            })
            .catch(error => {
                if(error !== undefined ){
    ...
    updatePayload = (id, payload) => {
        if (this._isMounted) {
            this.setState({ loadingSubmit: true });
            this.props.updateMetodePenyampaian(id, payload)
            .then((resp) => {
                toastr.success(resp.data.message);
                this.setState({ modalOpen: false,errors:[], loadingSubmit:false }, () => this.req())
            })
    ...
    deletePayload = (type) => {
         this.props.deleteMetodePenyampaian(type)
        .then((resp) => {
            toastr.success(resp.data.message);
            this.req()
        })
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">Metode Penyampaian</li>
                </ol>
                <h1 className="page-header">Metode Penyampaian<small> Master data Metode Penyampaian</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                    Metode Penyampaian
					</PanelHeader>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showMetodePenyampaian={this.props.showMetodePenyampaian}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showMetodePenyampaian: (id) => dispatch(showMetodePenyampaian(id)),
        deleteMetodePenyampaian: (id) => dispatch(deleteMetodePenyampaian(id)),
        saveMetodePenyampaian: (id, payload) => dispatch(saveMetodePenyampaian(id, payload)),
        updateMetodePenyampaian: (id, payload) => dispatch(updateMetodePenyampaian(id, payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(MetodePenyampaian));