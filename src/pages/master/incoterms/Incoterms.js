...
import { fetchIncoterms, showIncoterms, deleteIncoterms, saveIncoterms, updateIncoterms } from '../../../store/actions/master/incotermsActions';
...
class Incoterms extends Component {
    constructor(props) {
        ...
        this.columns = [
            {
                Header: ()=>this.props.t("incoterms:label.id"),
                accessor: "id",
                height: 10,
            ...
            {
                Header: () => this.props.t("incoterms:label.name"),
                accessor: "name",
                Filter: ({ filter, onChange }) => (
            ...
            {
                Header:  () => this.props.t("incoterms:label.status"),
                accessor: "status",
                Filter: ({ filter, onChange }) => (
            ...
            {
                Header:  () => this.props.t("incoterms:label.created_at"),
                id: "created_at",
            ...
            {
                Header: () => this.props.t("incoterms:label.updated_at"),
                id: "updated_at",
            ...
            {
                Header: ()=>this.props.t("incoterms:label.action"),
                filterable: false,
                sortable: false,
                accessor: "uuid",
                Cell: ({ value }) => (
    ...
    asyncData = async () => {
        ...
        this.props.fetchIncoterms(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (type,payload) => {
        if(type==="save"){
            ...
            this.props.saveIncoterms(payload)
            .then((resp) => {
    ...
    updatePayload = (type,id, payload) => {
        if (type === "update") {
            this.setState({loadingSubmit:true});
            this.props.updateIncoterms(id, payload)
            .then((resp) => {
                toastr.success(resp.data.message);
                this.setState({ modalOpen: false, loadingSubmit:false }, () => this.req())
                console.log(resp)
    ...
    deletePayload = (type) => {
         this.props.deleteIncoterms(type)
        .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">{t("incoterms:title")}</li>
                </ol>
                <h1 className="page-header">{t("incoterms:title")} <small>Master {t("incoterms:title")}</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        {t("incoterms:title")}
					</PanelHeader>
                        ...
                        <Row>
                            ...
                            <Col sm="6">
                                <div className="pull-right m-b-10">
                                    {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("incoterms:button.add")}</Button>}
                                </div>
                            </Col>
                        </Row>
                        <Row>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showIncoterms={this.props.showIncoterms}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showIncoterms: (id) => dispatch(showIncoterms(id)),
        deleteIncoterms: (id) => dispatch(deleteIncoterms(id)),
        saveIncoterms: (id, payload) => dispatch(saveIncoterms(id, payload)),
        updateIncoterms: (id, payload) => dispatch(updateIncoterms(id, payload)),
    }
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (Incoterms));
