...
import { fetchTermsOfPayment, showTermsOfPayment, deleteTermsOfPayment, saveTermsOfPayment, updateTermsOfPayment, syncTermsOfPayment } from '../../../store/actions/master/termsOfPaymentActions';
...
class TermsOfPayment extends Component {
    ...
    asyncData = async () => {
        ...
        this.props.fetchTermsOfPayment(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
	...
	handleSync = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.syncTermsOfPayment(val)
			.then(res => {
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.saveTermsOfPayment(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateTermsOfPayment(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteTermsOfPayment(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">Term Of Payment</li>
                </ol>
                <h1 className="page-header">Term Of Payment  <small>Master Data Term Of Payment</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        Term Of Payment
                    </PanelHeader>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showTermsOfPayment={this.props.showTermsOfPayment}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showTermsOfPayment: (id) => dispatch(showTermsOfPayment(id)),
        deleteTermsOfPayment: (id) => dispatch(deleteTermsOfPayment(id)),
        saveTermsOfPayment: (id, payload) => dispatch(saveTermsOfPayment(id, payload)),
        updateTermsOfPayment: (id, payload) => dispatch(updateTermsOfPayment(id, payload)),
		syncTermsOfPayment: (payload) => dispatch(syncTermsOfPayment(payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(TermsOfPayment));
