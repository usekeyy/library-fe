...
import { fetchDistricts, showDistricts, deleteDistricts, saveDistricts, updateDistricts } from '../../../store/actions/master/districtsActions';
...
class Districts extends Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            {
                Header: () => this.props.t("district:label.name") ,
                accessor: "name",
                Filter: ({ filter, onChange }) => (
    ...
    asyncData = async () => {
        ...
        this.props.fetchDistricts(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (payload) => {
        if (this._isMounted) {
            ...
            this.props.saveDistricts(payload)
                .then((resp) => {
    ...
    updatePayload = (id, payload) => {
        if (this._isMounted) {
            this.setState({ loadingSubmit: true });
            this.props.updateDistricts(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit:false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteDistricts(type)
            .then((resp) => {
                toastr.success(resp.data.message);
    ...
    render() {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">{t("district:title")}</li>
                </ol>
                <h1 className="page-header">{t("district:title")}  <small>Master {t("district:title")}</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        {t("district:title")}
					</PanelHeader>
        ...
        {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("district:button.add")}</Button>}
                                </div>
                            </Col>
                        </Row>
                        <Row>
...
const dispatchToProps = dispatch => {
    return {
        ...
        showDistricts: (id) => dispatch(showDistricts(id)),
        deleteDistricts: (id) => dispatch(deleteDistricts(id)),
        saveDistricts: (id, payload) => dispatch(saveDistricts(id, payload)),
        updateDistricts: (id, payload) => dispatch(updateDistricts(id, payload)),
...
export default connect(stateToProps, dispatchToProps)( withTranslation() (Districts));
