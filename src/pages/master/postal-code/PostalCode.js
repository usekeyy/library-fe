...
import { fetchPostcalCode, savePostcalCode , updatePostcalCode, showPostalCode, deletePostcalCode } from '../../../store/actions/master/postalCodeActions';
...
class PostalCode extends Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            {
                Header: () => this.props.t("postalCode:label.name"),
                accessor: "name",
                Filter: ({ filter, onChange }) => (
            ...
            {
                Header: () => this.props.t("postalCode:label.district-id"),
            ...
            {
                Header: () => this.props.t("postalCode:label.district-name"),
                accessor: "sub_districts_name",
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="sub_districts_name" onChange={(event) => this.onChanged(event)} value={this.state.params.sub_districts_name} />
                )
            },
            {
    ...
    asyncData = async () => {
        ...
        this.props.fetchPostcalCode(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (payload) => {
        if (this._isMounted) {
            ...
            this.props.savePostcalCode(payload)
                .then((resp) => {
    ...
    updatePayload = (id, payload) => {
        if (this._isMounted) {
            this.setState({ loadingSubmit: true });
            this.props.updatePostcalCode(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit:false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deletePostcalCode(type)
            .then((resp) => {
                toastr.success(resp.data.message);
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">{t("postalCode:title")}</li>
                </ol>
                <h1 className="page-header">{t("postalCode:title")}  <small>Master {t("postalCode:title")}</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        {t("postalCode:table-title")}
					</PanelHeader>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showPostalCode={this.props.showPostalCode}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showPostalCode: (id) => dispatch(showPostalCode(id)),
        deletePostcalCode: (id) => dispatch(deletePostcalCode(id)),
        savePostcalCode: (id, payload) => dispatch(savePostcalCode(id, payload)),
        updatePostcalCode: (id, payload) => dispatch(updatePostcalCode(id, payload)),
...
export default connect(stateToProps, dispatchToProps)( withTranslation() (PostalCode));
