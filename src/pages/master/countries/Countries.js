...
import { fetchCountries, showCountries, deleteCountries, saveCountries, updateCountries } from '../../../store/actions/master/countriesActions';
...
class Countries extends Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            {
                Header: () => this.props.t("uom:label.long-name"),
                accessor: "long_name",
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="long_name" onChange={(event) => this.onChanged(event)} value={this.state.params.long_name} />
                )
            },
            {
                Header: "Status",
    ...
    asyncData = async () => {
        ...
        this.props.fetchCountries(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.saveCountries(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateCountries(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteCountries(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">{t("country:title")}</li>
                </ol>
                <h1 className="page-header">{t("country:title")}  <small>Master Data {t("country:title")}</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        {t("country:table-title")}
                    </PanelHeader>
                        ...
                        <Row>
                            ...
                            <Col sm="6">
                                <div className="pull-right m-b-10">
                                    {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("country:button.add")}</Button>}
                                </div>
                            </Col>
                        </Row>
                        <Row>
...
const dispatchToProps = dispatch => {
    return {
        ...
        showCountries: (id) => dispatch(showCountries(id)),
        deleteCountries: (id) => dispatch(deleteCountries(id)),
        saveCountries: (id, payload) => dispatch(saveCountries(id, payload)),
        updateCountries: (id, payload) => dispatch(updateCountries(id, payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(Countries));
