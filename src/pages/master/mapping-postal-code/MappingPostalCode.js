...
import { fetchPostalCodeMapping, savePostalCodeMapping , updatePostalCodeMapping, showPostalCodeMapping, deletePostalCodeMapping } from '../../../store/actions/master/postalCodeActions';
...
class MappingPostalCode extends Component {
    constructor(props) {
        super(props)
        ...
        this.columns = [
            {
                Header: () => this.props.t("postalCode:label.id"),
                accessor: "id",
                height: 10,
            ...
            {
                Header: () => this.props.t("postalCode:label.country-id"),
                accessor: "country_id",
                height: 10,
                Filter: ({ filter, onChange }) => (
            ...
            {
                Header: () => this.props.t("postalCode:label.country-name"),
            ...
            },
            {
                ...
                accessor: "required",
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="required" onChange={(event) => this.onChanged(event)} value={this.state.params.required} />
                ),
                Cell: ({ value }) => (
            ...
            },
            {
                ...
                accessor: "length",
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="length_column" onChange={(event) => this.onChanged(event)} value={this.state.params.length_column} />
                ),
               
            },
            // {
            //     Header: () => this.props.t("postalCode:label.status") ,
            //     id: "status",
            //     accessor: d => d.status,
            //     Filter: ({ filter, onChange }) => (
            ...
            {
                Header: () => this.props.t("postalCode:label.updated_at") ,
                id: "updated_at",
    ...
    asyncData = async () => {
        ...
        this.props.fetchPostalCodeMapping(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    updatePayload = (id, payload) => {
        if (this._isMounted) {
            this.setState({ loadingSubmit: true });
            this.props.updatePostalCodeMapping(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit:false }, () => this.req())
    ...
    deletePayload = (type) => {
        this.props.deletePostalCodeMapping(type)
            .then((resp) => {
                toastr.success(resp.data.message);
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">Mapping Postal Code</li>
                </ol>
                <h1 className="page-header">Mapping Postal Code  <small>Master Mapping Postal Code</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        Tabel Mapping Postal Code
					</PanelHeader>
                        ...
                        <Row>
                            ...
                            <Col sm="6">
                                <div className="pull-right m-b-10">
                                    {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("postalCode:button.add")}</Button>}
                                    {/* <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("postalCode:button.add")}</Button> */}
                                </div>
                            </Col>
                        </Row>
                        <Row>
...
const dispatchToProps = dispatch => {
    return {
        fetchPostalCodeMapping: (params) => dispatch(fetchPostalCodeMapping(params)),
        showPostalCodeMapping: (id) => dispatch(showPostalCodeMapping(id)),
        deletePostalCodeMapping: (id) => dispatch(deletePostalCodeMapping(id)),
        savePostalCodeMapping: (id, payload) => dispatch(savePostalCodeMapping(id, payload)),
        updatePostalCodeMapping: (id, payload) => dispatch(updatePostalCodeMapping(id, payload)),
...
export default connect(stateToProps, dispatchToProps)( withTranslation() (MappingPostalCode));
