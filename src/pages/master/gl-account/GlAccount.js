...
import { fetchGlAccount , saveGlAccount , updateGlAccount , showGlAccount , deleteGlAccount } from '../../../store/actions/master/glAccountActions';
import { fetchChartOfAccount } from '../../../store/actions/master/chartOfAccountActions';
...
class GlAccount extends Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            {
                Header: () => this.props.t("uom:label.description"),
                accessor: "description",
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="description" onChange={(event) => this.onChanged(event)} value={this.state.params.name} />
                )
            },
            {
                Header: "Status",
    ...
    asyncData = async () => {
        ...
        this.props.fetchGlAccount(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (type,payload) => {
        if(type==="save"){
            ...
            this.props.saveGlAccount(payload)
            .then((resp) => {
    ...
    updatePayload = (type,id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateGlAccount(id, payload)
            .then((resp) => {
                toastr.success(resp.data.message);
                this.setState({ modalOpen: false, loadingSubmit:false }, () => this.req())
            })
    ...
    deletePayload = (type) => {
         this.props.deleteGlAccount(type)
        .then((resp) => {
            toastr.success(resp.data.message);
            this.req()
        })
    ...
    fetchChartOfAccount = () => {
        if(this._isMounted){
          this.setState(({ loadings }) => ({
            loadings: { ...loadings, chart_of_account: true }
          }));
          this.props.fetchChartOfAccount()
          .then((resp) => {
            ...
            this.setState(({ loadings }) => ({
              optionsChartOfAccount: options,
              loadings: { ...loadings, chart_of_account: false }
            }));
          })
          .catch((resp) => {
            this.setState(({ loadings }) => ({
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">Gl Account</li>
                </ol>
                <h1 className="page-header">Gl Account  <small>Master data Gl Account</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                    Gl Account
					</PanelHeader>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showGlAccount={this.props.showGlAccount}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showGlAccount: (id) => dispatch(showGlAccount(id)),
        deleteGlAccount: (id) => dispatch(deleteGlAccount(id)),
        saveGlAccount: (id, payload) => dispatch(saveGlAccount(id, payload)),
        updateGlAccount: (id, payload) => dispatch(updateGlAccount(id, payload)),
    }
}

export default connect(stateToProps, dispatchToProps) (withTranslation() (GlAccount))
