...
import {fetchNPWP,
				...
				saveNPWP,
				updateNPWP,
				syncNPWP} from '../../../store/actions/master/npwpActions';
...
class Bank extends Component {
    ...
    super(props);
		...
		this.columns = [
			...
			},
			{
				...
				accessor: d => d.city,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="city" onChange={(event) => this.handleFilter(event)} value={this.state.options.city} />
				)
			},
			{
				Header: "NPWP Number",
				id: "npwp_number",
				accessor: d => d.npwp_number,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="npwp_number" onChange={(event) => this.handleFilter(event)} value={this.state.options.npwp_number} />
				)
			},
			{
				Header: "NPWP Address",
				id: "npwp_address",
				accessor: d => d.npwp_address,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="npwp_address" onChange={(event) => this.handleFilter(event)} value={this.state.options.npwp_address} />
				)
			},
			{
				Header: "Utama",
				id: "flag",
				accessor: d => d.flag === 'y' ? "Utama" : "",
				Filter: ({ filter, onChange }) => (
						<FilterStatus statusSearch={this.state.flagSearch} getStatus={this.getCheckFlag} />
				)
			},
			{
			...
			},
			{
				...
				Cell: ({value}) => (
					<React.Fragment>
						<center>
							<Button color="warning" size="xs" value={value} onClick={(e) => this.toggleFormOpen(e, value)} ><span className="fa fa-edit"></span> </Button>
							{/* <Button color="danger" size="xs" value={value} onClick={(e) => this.toggleConfirm(e, value)} ><span className="fa fa-trash"></span></Button> */}
						</center>
					</React.Fragment>
				)
			},
  ...
  componentDidMount = () => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updateNPWP(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">NPWP</li>
				</ol>
				<h1 className="page-header">Master NPWP </h1>
				{/* <Button size="xs" className="btn btn-default">Sync</Button> */}
				<Panel loading={false}>
					<PanelHeader>{"Table Master NPWP"}</PanelHeader>
					<PanelBody loading={false}>
						<Row>
							<Col sm="6">
								<Button color="primary" size="sm" value="" onClick={this.syncNPWP} disabled={this.state.loadingSync}>
...
onSortedChange={val => {this.changeSorted(val) }}
				...
				fetchNPWP={this.props.fetchNPWP}
				showNPWP={this.props.showNPWP} 
...
const dispatchToProps = dispatch => {
	return {
		fetchNPWP: (params) => dispatch(fetchNPWP(params)),
		saveNPWP: (payload) => dispatch(saveNPWP(payload)),
		showNPWP: (id) => dispatch(showNPWP(id)),
		updateNPWP: (id, payload) => dispatch(updateNPWP(id, payload)),
		deleteNPWP: (id) => dispatch(deleteNPWP(id)),
		syncNPWP: (params) => dispatch(syncNPWP(params)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (Bank));