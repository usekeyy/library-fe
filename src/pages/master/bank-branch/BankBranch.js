...
import {fetchBankBranch,
				showBankBranch,
				deleteBankBranch,
				saveBankBranch,
				updateBankBranch} from '../../../store/actions/master/bankBranchActions';
...
class BankBranch extends Component {
    ...
    super(props);
		...
		this.columns = [
			{
				Header: () => this.props.t("uom:label.bank-name"),
				id: "bank_name",
				accessor: d => d.bank_name,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="bank_name" onChange={(event) => this.handleFilter(event)} value={this.state.options.bank_name} />
				)
			},
			{
				Header: () => this.props.t("uom:label.country"),
				id: "country_name",
				accessor: d => d.country_name,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="country_name" onChange={(event) => this.handleFilter(event)} value={this.state.options.country_name} />
				)
			},
			{
				Header: () => this.props.t("uom:label.district"),
				id: "district_name",
				accessor: d => d.district_name,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="district_name" onChange={(event) => this.handleFilter(event)} value={this.state.options.district_name} />
				)
			},
			{
				Header: () => this.props.t("uom:label.address"),
				id: "address",
				accessor: d => d.address,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="address" onChange={(event) => this.handleFilter(event)} value={this.state.options.address} />
				)
			},
			{
  ...
  componentDidMount = () => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updateBankBranch(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
	...
	handleDelete = (id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.deleteBankBranch(id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
...
handleFilter = (event) => {
	...
	fetchBank = (params) => {
		if(this._isMounted){
			// console.log(params);
			this.setState(({ loadings }) => ({ loadings: { ...loadings, bank: true }, m_bank: [] }));
			this.props.fetchBank(params)
			.then((resp) => {
				let m_bank = resp.data.data;
        let options = m_bank.map((dt) => {
          return { value: dt.id, label: dt.name };
			...
			.catch((resp) => {
				...
				this.setState(({ loadings }) => ({ loadings: { ...loadings, bank: false } }));
			});
		}
	}
	...
	fetchCountries = () => {
		if(this._isMounted){
			this.setState(({ loadings }) => ({ loadings: { ...loadings, country: true }, m_country: [] }));
        ...
        let options = m_country.map((dt) => {
          return { value: dt.id, label: dt.id+' - '+dt.name };
			...
			.catch((resp) => {
				...
				this.setState(({ loadings }) => ({ loadings: { ...loadings, country: false } }));
			});
		}
	}
	...
	fetchDistricts = (params) => {
		if(this._isMounted){
			// console.log(params);
			this.setState(({ loadings }) => ({ loadings: { ...loadings, district: true }, m_district: [] }));
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">{t("uom:title")}</li>
				</ol>
				<h1 className="page-header">BankBranch </h1>
				<Panel loading={false}>
					<PanelHeader>{t("uom:table-title-master.bank-branch")}</PanelHeader>
...
onSortedChange={val => {this.changeSorted(val) }}
				...
				fetchBankBranch={this.props.fetchBankBranch}
				showBankBranch={this.props.showBankBranch} 
...
const dispatchToProps = dispatch => {
	return {
		...
		saveBankBranch: (payload) => dispatch(saveBankBranch(payload)),
		showBankBranch: (id) => dispatch(showBankBranch(id)),
		updateBankBranch: (id, payload) => dispatch(updateBankBranch(id, payload)),
		deleteBankBranch: (id) => dispatch(deleteBankBranch(id)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (BankBranch));