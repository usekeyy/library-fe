...
import { fetchStorageLocation, saveStorageLocation, showStorageLocation, updateStorageLocation, deleteStorageLocation } from '../../../store/actions/master/storageLocationsActions';
...
class StorageLocation extends Component {
	constructor(props) {
		...
		this.columns = [
			{
				Header: () => this.props.t("storageLocation:label.code"),
				id: "code",
				accessor: d => d.code,
			...
			{
				Header: () => this.props.t("storageLocation:label.name"),
				id: "name",
				accessor: d => d.name,
				Filter: ({ filter, onChange }) => (
			...
			{
				Header: () => this.props.t("storageLocation:label.plant-id"),
				id: "plant_id",
			...
			{
				Header: () => this.props.t("storageLocation:label.plant-name"),
				id: "plant_name",
			...
			{
				Header: () => this.props.t("storageLocation:label.status"),
				id: "status",
			...
			{
				Header: () => this.props.t("storageLocation:label.created_at"),
				id: "created_at",
			...
			{
				Header: () => this.props.t("storageLocation:label.updated_at"),
				id: "updated_at",
			...
			{
				Header: () => this.props.t("storageLocation:label.action"),
				id: "action",
				accessor: "uuid",
				filterable: false,
				sortable: false,
	...
	asyncData = async (params) => {
		...
		this.props.fetchStorageLocation(params)
			.then((resp) => {
				if (this._isMounted) {
	...
	handleSave = (val) => {
		if (this._isMounted) {
			this.setState({ loading: true })
			this.props.saveStorageLocation(val)
				.then(res => {
	...
	handleUpdate = (payload, id) => {
		if (this._isMounted) {
			this.setState({ loading: true })
			this.props.updateStorageLocation(payload, id)
				.then(res => {
					const response = res.data;
					toastr.success(response.message);
	...
	handleDelete = (id) => {
		if (this._isMounted) {
			this.setState({ loading: true })
			this.props.deleteStorageLocation(id)
				.then(res => {
					const response = res.data;
					toastr.success(response.message);
		...
		return (
			<div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">{t("storageLocation:title")}</li>
				</ol>
				<h1 className="page-header">{t("storageLocation:title")} </h1>
				<Panel loading={false}>
					<PanelHeader>{t("storageLocation:table-title")}</PanelHeader>
						...
						<Row>
							...
							<Col sm="6">
								<div className="pull-right m-b-10">
									{this.props.access.C && <Button color="primary" size="sm" value="" onClick={(e) => this.toggleFormOpen(e)} >{t("storageLocation:button.add")}</Button>}
								</div>
							</Col>
						</Row>
						<ReactTable
				...
				{this.state.toggleAdd &&
					<ModalForm
						...
						fetchStorageLocation={this.props.fetchStorageLocation}
						showStorageLocation={this.props.showStorageLocation}
...
const dispatchToProps = dispatch => {
	return {
		...
		saveStorageLocation: (payload) => dispatch(saveStorageLocation(payload)),
		showStorageLocation: (id) => dispatch(showStorageLocation(id)),
		updateStorageLocation: (id, payload) => dispatch(updateStorageLocation(id, payload)),
		deleteStorageLocation: (id) => dispatch(deleteStorageLocation(id)),
		fetchPlant: (id) => dispatch(fetchPlant(id)),
	}
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(StorageLocation));