...
import { fetchDocumentType, showDocumentType, deleteDocumentType, saveDocumentType, updateDocumentType, syncDocumentType } from '../../../store/actions/master/documentTypeActions';
...
class DocumentType extends Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            {
				Header: () => this.props.t("documentType:label.company_id"),
                accessor: "company_id",
                Filter: ({ filter, onChange }) => (
            ...
            {
				Header: () => this.props.t("documentType:label.document_category"),
                accessor: "doc_cat",
                Filter: ({ filter, onChange }) => (
    ...
    asyncData = async () => {
        ...
        this.props.fetchDocumentType(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
	...
	handleSync = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.syncDocumentType(val)
			.then(res => {
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.saveDocumentType(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateDocumentType(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteDocumentType(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">{t("documentType:title")}</li>
                </ol>
                <h1 className="page-header">{t("documentType:title")}  <small>Master Data {t("documentType:title")}</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        {t("documentType:table-title")}
                    </PanelHeader>
...
const dispatchToProps = dispatch => {
    return {
        ...
        showDocumentType: (id) => dispatch(showDocumentType(id)),
        deleteDocumentType: (id) => dispatch(deleteDocumentType(id)),
        saveDocumentType: (id, payload) => dispatch(saveDocumentType(id, payload)),
        updateDocumentType: (id, payload) => dispatch(updateDocumentType(id, payload)),
		syncDocumentType: (payload) => dispatch(syncDocumentType(payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(DocumentType));
