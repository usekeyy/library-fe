...
import { fetchVprRating, showVprRating, deleteVprRating, saveVprRating, updateVprRating } from '../../../store/actions/master/vprActions';
...
class VprRating extends Component {
    constructor(props) {
        super(props)
    ...
    asyncData = async () => {
        ...
        this.props.fetchVprRating(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.saveVprRating(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateVprRating(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteVprRating(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">VPR Rating</li>
                </ol>
                <h1 className="page-header">VPR Rating  <small>Master Data VPR Rating</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        VPR Rating
                    </PanelHeader>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showVprRating={this.props.showVprRating}
...
const dispatchToProps = dispatch => {
    return {
        fetchVprRating: (params) => dispatch(fetchVprRating(params)),
        showVprRating: (id) => dispatch(showVprRating(id)),
        deleteVprRating: (id) => dispatch(deleteVprRating(id)),
        saveVprRating: (id, payload) => dispatch(saveVprRating(id, payload)),
        updateVprRating: (id, payload) => dispatch(updateVprRating(id, payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(VprRating));
