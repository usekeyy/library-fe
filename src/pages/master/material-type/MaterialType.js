...
import {fetchMaterialType,
				showMaterialType,
				deleteMaterialType,
				saveMaterialType,
				updateMaterialType} from '../../../store/actions/master/materialTypeActions.js';
...
class MaterialType extends Component {
    ...
    super(props);
		...
		this.columns = [
			...
			},
			{
				...
				accessor: d => d.flag,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="flag" onChange={(event) => this.handleFilter(event)} value={this.state.options.flag} />
				)
			},
			{
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
      ...
      this.props.fetchMaterialType(params)
      .then((resp) => {
				if(this._isMounted){
	...
	handleSave = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.saveMaterialType(val)
			.then(res => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updateMaterialType(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
	...
	handleDelete = (id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.deleteMaterialType(id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">{"MaterialType"}</li>
				</ol>
				<h1 className="page-header">MaterialType </h1>
				<Panel loading={false}>
					<PanelHeader>{"Table MaterialType"}</PanelHeader>
...
onSortedChange={val => {this.changeSorted(val) }}
				...
				showMaterialType={this.props.showMaterialType} 
...
const dispatchToProps = dispatch => {
	return {
		...
		saveMaterialType: (payload) => dispatch(saveMaterialType(payload)),
		showMaterialType: (id) => dispatch(showMaterialType(id)),
		updateMaterialType: (id, payload) => dispatch(updateMaterialType(id, payload)),
		deleteMaterialType: (id) => dispatch(deleteMaterialType(id)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (MaterialType));