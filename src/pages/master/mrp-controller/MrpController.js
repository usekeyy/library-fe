...
import {fetchMrpController, saveMrpController, showMrpController, updateMrpController, deleteMrpController} from '../../../store/actions/master/mrpControllerActions';
...
class MrpController extends Component {
	constructor(props) {
    super(props);
		this._isMounted = false;
		this.columns = [
			{
				Header: () => this.props.t("mrpController:label.code"),
				id: "code",
				accessor: d => d.code,
			...
			{
				Header: () => this.props.t("mrpController:label.name"),
				id: "name",
				accessor: d => d.name,
				Filter: ({ filter, onChange }) => (
			...
			{
				Header: () => this.props.t("mrpController:label.plant-id"),
				id: "plant_id",
			...
			{
				Header: () => this.props.t("mrpController:label.plant-name"),
				id: "plant_name",
			...
			{
				Header: () => this.props.t("mrpController:label.status"),
				id: "status",
			...
			{
				Header:() => this.props.t("mrpController:label.created_at"),
				id: "created_at",
			...
			{
				Header:() => this.props.t("mrpController:label.updated_at"),
				id: "updated_at",
			...
			{
				Header: () => this.props.t("mrpController:label.action"),
				id:"action",
				accessor: "uuid",
				filterable: false,
				sortable: false,
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
      ...
      this.props.fetchMrpController(params)
      .then((resp) => {
				if(this._isMounted){
	...
	handleSave = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.saveMrpController(val)
			.then(res => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updateMrpController(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
	...
	handleDelete = (id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.deleteMrpController(id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">Mrp Controller</li>
				</ol>
				<h1 className="page-header">{t("mrpController:title")}</h1>
				<Panel loading={false}>
					<PanelHeader>{t("mrpController:table-title")}</PanelHeader>
						...
						<Row>
							...
							<Col sm="6">
								<div className="pull-right m-b-10">
									{this.props.access.C && <Button color="primary" size="sm" value="" onClick={(e) => this.toggleFormOpen(e)} >{t("mrpController:button.add")}</Button>}
								</div>
							</Col>
						</Row>
						<ReactTable 
...
onSortedChange={val => {this.changeSorted(val) }}
				...
				fetchMrpController={this.props.fetchMrpController}
				showMrpController={this.props.showMrpController} 
...
const dispatchToProps = dispatch => {
	return {
		...
		saveMrpController: (payload) => dispatch(saveMrpController(payload)),
		showMrpController: (id) => dispatch(showMrpController(id)),
		updateMrpController: (id, payload) => dispatch(updateMrpController(id, payload)),
		deleteMrpController: (id) => dispatch(deleteMrpController(id)),
		fetchPlant: (payload) => dispatch(fetchPlant(payload)),
	}
}

export default connect(stateToProps, dispatchToProps) ( withTranslation() (MrpController));