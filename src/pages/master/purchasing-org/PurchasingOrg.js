...
import {fetchPurchasingOrg, savePurchasingOrg, showPurchasingOrg, updatePurchasingOrg, deletePurchasingOrg} from '../../../store/actions/master/purchasingOrgActions';
...
class PurchasingOrg extends Component {
	constructor(props) {
    super(props);
		this._isMounted = false;
		this.columns = [
			{
				Header: () => this.props.t("purchasingOrg:label.id"),
				id: "id",
				accessor: d => d.id,
			...
			{
				Header: () => this.props.t("purchasingOrg:label.name"),
				id: "name",
				accessor: d => d.name,
				Filter: ({ filter, onChange }) => (
			...
			{
				Header: () => this.props.t("purchasingOrg:label.company-id"),
				id: "company_id",
			...
			{
				Header:  () => this.props.t("purchasingOrg:label.company-name"),
				id: "company_name",
				accessor: d => d.company_name,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="conpany_name" onChange={(event) => this.handleFilter(event)} value={this.state.options.conpany_name} />
				)
			},
			{
				Header:  () => this.props.t("purchasingOrg:label.status"),
				id: "status",
			...
			{
				Header:  () => this.props.t("purchasingOrg:label.created_at"),
				id: "created_at",
			...
			{
				Header: () => this.props.t("purchasingOrg:label.updated_at"),
				id: "updated_at",
			...
			{
				Header:() => this.props.t("purchasingOrg:label.action"),
				id:"action",
				accessor: "uuid",
				filterable: false,
				sortable: false,
  ...
  componentDidMount = () => {
	...
	handleSave = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.savePurchasingOrg(val)
			.then(res => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updatePurchasingOrg(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
	...
	handleDelete = (id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.deletePurchasingOrg(id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">Purchasing Org</li>
				</ol>
				<h1 className="page-header">{t("purchasingOrg:title")}</h1>
				<Panel loading={false}>
					<PanelHeader>{t("purchasingOrg:table-title")}</PanelHeader>
						...
						<Row>
							...
							<Col sm="6">
								<div className="pull-right m-b-10">
									{this.props.access.C && <Button color="primary" size="sm" value="" onClick={(e) => this.toggleFormOpen(e)} >{t("purchasingOrg:button.add")}</Button>}
								</div>
							</Col>
						</Row>
						<ReactTable 
...
onSortedChange={val => {this.changeSorted(val) }}
				...
				showPurchasingOrg={this.props.showPurchasingOrg} 
...
const dispatchToProps = dispatch => {
	return {
		...
		savePurchasingOrg: (payload) => dispatch(savePurchasingOrg(payload)),
		showPurchasingOrg: (id) => dispatch(showPurchasingOrg(id)),
		updatePurchasingOrg: (id, payload) => dispatch(updatePurchasingOrg(id, payload)),
		deletePurchasingOrg: (id) => dispatch(deletePurchasingOrg(id)),
		fetchCompanies: (params) => dispatch(fetchCompanies(params)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (PurchasingOrg));