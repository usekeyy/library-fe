...
import { fetchBidangUsaha , saveBidangUsaha , showBidangUsaha, deleteBidangUsaha, updateBidangUsaha, syncBidangUsaha } from '../../../store/actions/master/bidangUsahaActions';
...
class VendorBidangUsaha extends Component {
    constructor(props) {
        super(props)
        ...
        this.columns = [
            {
                Header: () => this.props.t("vendorBidangUsaha:label.id"),
                accessor: "id",
                height: 10,
            ...
            {
                Header: () => this.props.t("vendorBidangUsaha:label.name"),
                accessor: "name",
                Filter: ({ filter, onChange }) => (
            ...
            },
            {
                Header: "Construction",
                id : "construction",
                accessor: d => d.construction === "y" ? "Ya" : "Tidak",
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="construction" onChange={(event) => this.onChanged(event)} value={this.state.params.construction} />
                )
            },
            {
                Header: () => this.props.t("vendorBidangUsaha:label.status"),
                id: "status",
                accessor: d => d.status,
                Filter: ({ filter, onChange }) => (
            ...
            {
                Header: () => this.props.t("vendorBidangUsaha:label.created_at"),
                id: "created_at",
            ...
            {
                Header: () => this.props.t("vendorBidangUsaha:label.updated_at"),
                id: "updated_at",
            ...
            {
                Header: () => this.props.t("vendorBidangUsaha:label.action"),
                filterable: false,
                sortable: false,
                accessor: "uuid",
                Cell: ({ value }) => (
    ...
    asyncData = async () => {
        ...
        this.props.fetchBidangUsaha(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
	...
	handleSync = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.syncBidangUsaha(val)
			.then(res => {
    ...
    savePayload = (payload) => {
        if (this._isMounted) {
            ...
            this.props.saveBidangUsaha(payload)
                .then((resp) => {
    ...
    updatePayload = (id, payload) => {
        if (this._isMounted) {
            this.setState({ loadingSubmit: true });
            this.props.updateBidangUsaha(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteBidangUsaha(type)
            .then((resp) => {
                toastr.success(resp.data.message);
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">{t("vendorBidangUsaha:title")}</li>
                </ol>
                <h1 className="page-header">{t("vendorBidangUsaha:title")}  <small>Master {t("vendorBidangUsaha:title")}</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        {t("vendorBidangUsaha:table-title")}
                    </PanelHeader>
                        ...
                        <Row>
                            ...
                            <Col sm="6">
                                <div className="pull-right m-b-10">
                                    {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("vendorBidangUsaha:button.add")}</Button>}
                                </div>
                                <span> </span>
                                <div className="pull-right m-b-10">
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showBidangUsaha={this.props.showBidangUsaha}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showBidangUsaha: (id) => dispatch(showBidangUsaha(id)),
        deleteBidangUsaha: (id) => dispatch(deleteBidangUsaha(id)),
        saveBidangUsaha: (id, payload) => dispatch(saveBidangUsaha(id, payload)),
        updateBidangUsaha: (id, payload) => dispatch(updateBidangUsaha(id, payload)),
        syncBidangUsaha: (payload) => dispatch(syncBidangUsaha(payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(VendorBidangUsaha));
