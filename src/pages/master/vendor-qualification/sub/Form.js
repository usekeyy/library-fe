		...
		<div>
			<form onSubmit={handleSubmit(onSubmit)}>
				<ModalBody>
					...
					<div className="form-group">
						<label>{t("vendorQualification:label.name")} <span className="text-danger">*</span></label>
				...
				<ModalFooter>
					<button className="btn btn-white" onClick={() => props.toggleClose()}>{t("vendorQualification:button.close")}</button>
					<button className="btn btn-success" type="submit" disabled={props.loadingSubmit}>{props.loadingSubmit ? <i className="fa fa-spinner fa-spin"></i> : ''} {props.uuid !== "" ? t("vendorQualification:button.update") : t("vendorQualification:button.submit")}</button>
				</ModalFooter>
			</form>
		</div>
	);
