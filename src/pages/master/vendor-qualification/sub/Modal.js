...
class ModalForm extends Component {
  ...
  getUUID() {
    if (this._isMounted) {
      ...
      this.props
        .showVendorQualification(this.props.uuid)
        .then((resp) => {
  ...
  handleSave = (payload) => {
    if (this._isMounted) {
      if (this.props.uuid !== "") {
        this.props.update(this.props.uuid, {name : payload.name});
      } else {
      ...
      <div>
        <Modal isOpen={this.props.toggleAdd} toggle={() => this.toggleClose()}>
          <ModalHeader toggle={() => this.toggleClose()}>
            {this.props.uuid !== "" ? t("vendorQualification:modal.title-update") : t("vendorQualification:modal.title-create")}
          </ModalHeader>
          {this.state.loading && (
            <center>
              <br />
