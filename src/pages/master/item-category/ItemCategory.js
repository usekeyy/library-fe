...
import {fetchItemCategory,
				showItemCategory,
				deleteItemCategory,
				saveItemCategory,
				updateItemCategory} from '../../../store/actions/master/itemCategoryActions';
...
class ItemCategory extends Component {
    ...
    super(props);
		...
		this.columns = [
			...
			{
				Header: () => this.props.t("uom:label.symbol"),
				id: "symbol",
				accessor: d => d.symbol,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="symbol" onChange={(event) => this.handleFilter(event)} value={this.state.options.symbol} />
				)
			},
			{
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
      ...
      this.props.fetchItemCategory(params)
      .then((resp) => {
				if(this._isMounted){
	...
	handleSave = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.saveItemCategory(val)
			.then(res => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updateItemCategory(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
	...
	handleDelete = (id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.deleteItemCategory(id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
  ...
  render(){
    ...
    return (
      <div>
				...
				<h1 className="page-header">Item Category </h1>
				<Panel loading={false}>
					<PanelHeader>{"Table Item Category"}</PanelHeader>
...
onSortedChange={val => {this.changeSorted(val) }}
				...
				fetchItemCategory={this.props.fetchItemCategory}
				showItemCategory={this.props.showItemCategory} 
...
const dispatchToProps = dispatch => {
	return {
		...
		saveItemCategory: (payload) => dispatch(saveItemCategory(payload)),
		showItemCategory: (id) => dispatch(showItemCategory(id)),
		updateItemCategory: (id, payload) => dispatch(updateItemCategory(id, payload)),
		deleteItemCategory: (id) => dispatch(deleteItemCategory(id)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (ItemCategory));