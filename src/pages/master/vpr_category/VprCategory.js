...
import { fetchVprCategory, showvVprCategory, deleteVprCategory, saveVprCategory, updateVprCategory } from '../../../store/actions/master/vprActions';
...
class VprCategory extends Component {
    constructor(props) {
        super(props)
    ...
    asyncData = async () => {
        ...
        this.props.fetchVprCategory(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.saveVprCategory(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateVprCategory(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteVprCategory(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">VPR Category</li>
                </ol>
                <h1 className="page-header">VPR Category  <small>Master Data VPR Category</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        VPR Category
                    </PanelHeader>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showvVprCategory={this.props.showvVprCategory}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showvVprCategory: (id) => dispatch(showvVprCategory(id)),
        deleteVprCategory: (id) => dispatch(deleteVprCategory(id)),
        saveVprCategory: (id, payload) => dispatch(saveVprCategory(id, payload)),
        updateVprCategory: (id, payload) => dispatch(updateVprCategory(id, payload)),
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(VprCategory));
