...
import {fetchMaintPlannerGroup, saveMaintPlannerGroup, showMaintPlannerGroup, updateMaintPlannerGroup, deleteMaintPlannerGroup} from '../../../store/actions/master/maintPlannerGroupActions';
...
class MaintPlannerGroup extends Component {
	constructor(props) {
    super(props);
		this._isMounted = false;
		this.columns = [
			{
				Header: () => this.props.t("maintPlannerGroup:label.code"),
				id: "code",
				accessor: d => d.code,
			...
			{
				Header: () => this.props.t("maintPlannerGroup:label.name"),
				id: "name",
				accessor: d => d.name,
				Filter: ({ filter, onChange }) => (
			...
			{
				Header: () => this.props.t("maintPlannerGroup:label.plant-id"),
				id: "plant_id",
				accessor: d => d.plant_id,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="plant_id" onChange={(event) => this.handleFilter(event)} value={this.state.options.plant_id} />
				)
			},
			{
				Header: () => this.props.t("maintPlannerGroup:label.plant-name"),
				id: "plant_name",
				accessor: d => d.plant_name,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="plant_name" onChange={(event) => this.handleFilter(event)} value={this.state.options.plant_name} />
				)
			},
			{
				Header:() => this.props.t("maintPlannerGroup:label.status"),
				id: "status",
			...
			{
				Header:() => this.props.t("maintPlannerGroup:label.created_at"),
				id: "created_at",
			...
			{
				Header: () => this.props.t("maintPlannerGroup:label.updated_at"),
				id: "updated_at",
			...
			{
				Header: () => this.props.t("maintPlannerGroup:label.action"),
				id:"action",
				accessor: "uuid",
				filterable: false,
				sortable: false,
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
      ...
      this.props.fetchMaintPlannerGroup(params)
      .then((resp) => {
				if(this._isMounted){
	...
	handleSave = (val) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.saveMaintPlannerGroup(val)
			.then(res => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updateMaintPlannerGroup(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
	...
	handleDelete = (id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.deleteMaintPlannerGroup(id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">{t("maintPlannerGroup:title")}</li>
				</ol>
				<h1 className="page-header">{t("maintPlannerGroup:title")} </h1>
				<Panel loading={false}>
					<PanelHeader>{t("maintPlannerGroup:table-title")}</PanelHeader>
						...
						<Row>
							...
							<Col sm="6">
								<div className="pull-right m-b-10">
									{this.props.access.C && <Button color="primary" size="sm" value="" onClick={(e) => this.toggleFormOpen(e)} >{t("maintPlannerGroup:button.add")}</Button>}
								</div>
							</Col>
						</Row>
						<ReactTable 
...
onSortedChange={val => {this.changeSorted(val) }}
				...
				fetchMaintPlannerGroup={this.props.fetchMaintPlannerGroup}
				showMaintPlannerGroup={this.props.showMaintPlannerGroup} 
...
const dispatchToProps = dispatch => {
	return {
		fetchMaintPlannerGroup: (params) => dispatch(fetchMaintPlannerGroup(params)),
		saveMaintPlannerGroup: (payload) => dispatch(saveMaintPlannerGroup(payload)),
		showMaintPlannerGroup: (id) => dispatch(showMaintPlannerGroup(id)),
		updateMaintPlannerGroup: (id, payload) => dispatch(updateMaintPlannerGroup(id, payload)),
		deleteMaintPlannerGroup: (id) => dispatch(deleteMaintPlannerGroup(id)),
		fetchPlant: (params) => dispatch(fetchPlant(params)),
	}
}

export default connect(stateToProps, dispatchToProps)(withTranslation() (MaintPlannerGroup));