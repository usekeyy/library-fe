...
import { fetchCompanyType , saveCompanyType , updateCompanyType , deleteCompanyType , showCompanyType } from '../../../store/actions/master/vendorCompanyTypeActions';
...
class CompanyType extends Component {
    constructor(props) {
        ...
        this.columns = [
            {
                Header: () => this.props.t("companyType:label.id"),
                accessor: "id",
                height: 10,
            ...
            {
                Header: () => this.props.t("companyType:label.name"),
                accessor: "name",
                Filter: ({ filter, onChange }) => (
            ...
            {
                Header: () => this.props.t("companyType:label.description"),
            ...
            {
                Header: () => this.props.t("companyType:label.status"),
                id: "status",
                accessor: d => d.status,
                Filter: ({ filter, onChange }) => (
            ...
            {
                Header: () => this.props.t("companyType:label.created_at"),
                id: "created_at",
            ...
            {
                Header: () => this.props.t("companyType:label.updated_at"),
                id: "updated_at",
            ...
            // {
            //     Header: () => this.props.t("companyType:label.action"),
            //     filterable: false,
            //     sortable: false,
            //     accessor: "uuid",
            //     Cell: ({ value }) => (
    ...
    asyncData = async () => {
        ...
        this.props.fetchCompanyType(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (payload) => {
        if (this._isMounted) {
            ...
            this.props.saveCompanyType(payload)
                .then((resp) => {
    ...
    updatePayload = (id, payload) => {
        if (this._isMounted) {
            this.setState({ loadingSubmit: true });
            this.props.updateCompanyType(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteCompanyType(type)
            .then((resp) => {
                toastr.success(resp.data.message);
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">{t("companyType:title")}</li>
                </ol>
                <h1 className="page-header">{t("companyType:title")}  <small>Master {t("companyType:title")}</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        {t("companyType:title")}
                    </PanelHeader>
                        ...
                        {/* <Row>
                            ...
                            <Col sm="6">
                                <div className="pull-right m-b-10">
                                    {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("companyType:button.add")}</Button>}
                                </div>
                            </Col>
                        </Row> */}
                        <Row>
                ...
                {this.state.modalOpen &&
                    <ModalForm
                        ...
                        showCompanyType={this.props.showCompanyType}
...
const dispatchToProps = dispatch => {
    return {
        ...
        showCompanyType: (id) => dispatch(showCompanyType(id)),
        deleteCompanyType: (id) => dispatch(deleteCompanyType(id)),
        saveCompanyType: (id, payload) => dispatch(saveCompanyType(id, payload)),
        updateCompanyType: (id, payload) => dispatch(updateCompanyType(id, payload)),
       
    }
}

export default connect(stateToProps, dispatchToProps)(withTranslation()(CompanyType));
