		...
		<div>
			<form onSubmit={handleSubmit(onSubmit)}>
				<ModalBody>
					...
					<div className="form-group">
						<label>Category VPR <span className="text-danger">*</span></label>
						<div>
							<Controller
								...
								as={Select}
								control={control}
								options={props.m_category}
								defaultValue={props.data.vpr_category_id}
								inputRef={(e) => register({ name: "vpr_category_id", required: true })}
								name="vpr_category_id"
								placeholder={ t("common:Select.Pilih") }
								isLoading={ props.loading_category }
								rules={{ required: false }}
							/>
							{errors.vpr_category_id && <span className="text-danger">* This field is required</span>}
							{msg.vpr_category_id && <span className="text-danger">{msg.vpr_category_id[0]}</span>}
						</div>
					</div>
					<div className="form-group">
