...
import {fetchUserSap,
				...
				saveUserSap,
				updateUserSap,
				fetchUser} from '../../../store/actions/master/userHasSapActions';
...
class UserHasSap extends Component {
	constructor(props) {
    super(props);
		...
		this.columns = [
			...
			// },
			{
				...
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="user_id" onChange={(event) => this.handleFilter(event)} value={this.state.options.user_id} />
				)
			},
			{
				Header:"Name",
				...
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="user_name" onChange={(event) => this.handleFilter(event)} value={this.state.options.user_name} />
				)
			},
			{
				Header:"User SAP",
				id: "user_sap",
				accessor: d => d.user_sap,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="user_sap" onChange={(event) => this.handleFilter(event)} value={this.state.options.user_sap} />
				)
			},
			{
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
      ...
      this.props.fetchUserSap(params)
      .then((resp) => {
				if(this._isMounted){
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updateUserSap(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">{"Mapping User SAP"}</li>
				</ol>
				<h1 className="page-header">Mapping User SAP </h1>
				<Panel loading={false}>
					<PanelHeader>{"Table Mapping User SAP"}</PanelHeader>
...
onSortedChange={val => {this.changeSorted(val) }}
				...
				fetchUserSap={this.props.fetchUserSap}
				showUserSap={this.props.showUserSap} 
...
const dispatchToProps = dispatch => {
	return {
		fetchUserSap: (params) => dispatch(fetchUserSap(params)),
		saveUserSap: (payload) => dispatch(saveUserSap(payload)),
		showUserSap: (id) => dispatch(showUserSap(id)),
		updateUserSap: (id, payload) => dispatch(updateUserSap(id, payload)),
		deleteUserSap: (id) => dispatch(deleteUserSap(id)),
		fetchUser: (params) => dispatch(fetchUser(params)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (UserHasSap));