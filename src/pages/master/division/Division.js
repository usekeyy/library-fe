...
import { fetchDivision, showDivision, deleteDivision, saveDivision, updateDivision } from '../../../store/actions/master/divisionActions';
...
class Division extends Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            {
				Header: () => this.props.t("division:label.name"),
                accessor: "name",
            ...
            {
				Header: () => this.props.t("division:label.purchasing_org_id"),
            ...
            {
				Header: () => this.props.t("division:label.chief"),
                accessor: "chief_name",
                Filter: ({ filter, onChange }) => (
                    <input className="form-control" name="chief_name" onChange={(event) => this.onChanged(event)} value={this.state.params.chief_name} />
                )
            },
            {
				Header: () => this.props.t("division:label.status"),
                id: "status",
                accessor: d => d.status,
                Filter: ({ filter, onChange }) => (
            ...
            {
				Header: () => this.props.t("division:label.created_at"),
                id: "created_at",
            ...
            {
				Header: () => this.props.t("division:label.updated_at"),
                id: "updated_at",
            ...
            {
				Header: () => this.props.t("division:label.action"),
                filterable: false,
                sortable: false,
                accessor: "uuid",
                Cell: ({ value }) => (
    ...
    asyncData = async () => {
        ...
        this.props.fetchDivision(this.state.params)
            .then((resp) => {
                this.setState({
                    data: resp.data.data,
    ...
    savePayload = (type, payload) => {
        if (type === "save") {
            ...
            this.props.saveDivision(payload)
                .then((resp) => {
    ...
    updatePayload = (type, id, payload) => {
        if (type === "update") {
            this.setState({ loadingSubmit: true });
            this.props.updateDivision(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, modalDetail: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
    ...
    deletePayload = (type) => {
        this.props.deleteDivision(type)
            .then((resp) => {
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
                    ...
                    <li className="breadcrumb-item active">{t("division:title")}</li>
                </ol>
                <h1 className="page-header">{t("division:title")}  <small>Master Data {t("division:title")}</small></h1>
                <Panel loading={false}>
                    <PanelHeader>
                        {t("division:table-title")}
                    </PanelHeader>
                        ...
                        <Row>
                            ...
                            <Col sm="6">
                                <div className="pull-right m-b-10">
                                    {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("division:button.add")}</Button>}
                                </div>
                            </Col>
                        </Row>
                        <Row>
...
const dispatchToProps = dispatch => {
    return {
        ...
        showDivision: (id) => dispatch(showDivision(id)),
        deleteDivision: (id) => dispatch(deleteDivision(id)),
        saveDivision: (id, payload) => dispatch(saveDivision(id, payload)),
        updateDivision: (id, payload) => dispatch(updateDivision(id, payload)),
...
export default connect(stateToProps, dispatchToProps)(withTranslation()(Division));
