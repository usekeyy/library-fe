...
import {fetchBank,
				...
				deleteBank,
				saveBank,
				updateBank} from '../../../store/actions/master/bankActions';
...
class Bank extends Component {
  ...
  componentDidMount = () => {
	...
	handleUpdate = (payload, id) => {
		if(this._isMounted){
			this.setState({loading: true})
			this.props.updateBank(payload, id)
			.then(res => {
				const response = res.data;
				toastr.success(response.message);
  ...
  render(){
    ...
    return (
      <div>
				...
				<h1 className="page-header">Bank </h1>
				<Panel loading={false}>
					<PanelHeader>{"Table Bank"}</PanelHeader>
...
const dispatchToProps = dispatch => {
	return {
		...
		saveBank: (payload) => dispatch(saveBank(payload)),
		showBank: (id) => dispatch(showBank(id)),
		updateBank: (id, payload) => dispatch(updateBank(id, payload)),
