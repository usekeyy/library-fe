...
class Table extends Component {
		...
		this.columns = [
			...
			},
			{
				...
				Cell: ({value}) => (
					<React.Fragment>
						<center>
							{<Button color="primary" size="xs" value={value} onClick={(e) => this.showDetail(e, value)} >Proses </Button>}
						</center>
					</React.Fragment>
				)
			}
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
      ...
      this.props.fetchVerificationRequestor(params)
      .then((resp) => {
				if(this._isMounted){
...
handleFilter = (event) => {
	...
	showDetail = (e, value) => {
		// e.preventDefault();
		this.props.parentProps.history.push('/vendor/verification-registration/detail/'+value)
	}
