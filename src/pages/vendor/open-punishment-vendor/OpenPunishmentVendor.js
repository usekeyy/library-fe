...
import {fetchOpenPunishmentVendor, showOpenPunishmentVendor, saveOpenPunishmentVendor, updateOpenPunishmentVendor} from '../../../store/actions/vendor/openPunishmentVendorActions';
...
class OpenPunishmentVendor extends Component {
    constructor(props) {
        super(props)
    ...
    updatePayload = (id, payload) => {
        if (this._isMounted) {
            this.setState({ loadingSubmit: true });
            this.props.updateOpenPunishmentVendor(id, payload)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.setState({ modalOpen: false, modalDetail: false, loadingSubmit: false }, () => this.req())
                    console.log(resp)
        ...
        return (
            <div>
                <ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">{t("openPunishmentVendor:label.open_punishment_vendor")}</li>
                </ol>
                <h1 className="page-header">{t("openPunishmentVendor:label.open_punishment_vendor")}</h1>
                <Panel loading={false}>
                    <PanelHeader>{t("openPunishmentVendor:table")} {t("openPunishmentVendor:label.open_punishment_vendor")}</PanelHeader>
                            ...
                            { !this.state.modalOpen ?
                                <Col sm="6">
                                    <div className="pull-right m-b-10">
                                        {this.props.access.C && <Button color="primary" className="btn btn-sm btn-primary" value='' onClick={(e) => this.toggleFormOpen(e, '')} >{t("openPunishmentVendor:button.add")}</Button>}
                                    </div>
                                </Col> : 
                                <Col sm="6">
                                    <div className="pull-right m-b-10">
...
const dispatchToProps = dispatch => {
	return {
		...
		saveOpenPunishmentVendor: (params) => dispatch(saveOpenPunishmentVendor(params)),
		updateOpenPunishmentVendor: (id, payload) => dispatch(updateOpenPunishmentVendor(id, payload)),
...
export default connect(stateToProps, dispatchToProps)( withTranslation() (OpenPunishmentVendor));