...
import {fetchDataVendor, downloadProfilVendor} from '../../../store/actions/vendor/vendorActions';
...
class VendorRekanan extends Component {
	constructor(props) {
    super(props);
  ...
  componentDidMount = () => {
	...
	downloadProfilVendor = (e, uuid, nama_vendor) => {
			e.preventDefault()
			this.props.downloadProfilVendor(uuid)
			.then((resp) => {
			const url = window.URL.createObjectURL(new Blob([resp.data]));
			const link = document.createElement('a');
			link.href = url;
			link.setAttribute('download', `Profil_Vendor_${nama_vendor}.xlsx`); //or any other extension
    ...
    return (
      <div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">Daftar Vendor</li>
				</ol>
				<h1 className="page-header">Daftar Vendor </h1>
				<Panel loading={false}>
					<PanelHeader>Daftar Vendor Table</PanelHeader>
					<PanelBody loading={false}>
						...
						{!form && <Table 
							...
							downloadProfilVendor={this.downloadProfilVendor}
						/>}
					</PanelBody>
				</Panel>
			</div>
...
const dispatchToProps = dispatch => {
	return {
		...
		downloadProfilVendor:(uuid) => dispatch(downloadProfilVendor(uuid))
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (VendorRekanan));