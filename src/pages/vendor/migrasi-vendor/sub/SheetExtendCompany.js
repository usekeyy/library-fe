...
import {fetchMigrasiExtendCompany} from '../../../../store/actions/vendor/migrasiVendorActions';
import {debounce} from '../../../../helpers/debounce';
const ReactTableFixedColumns = withFixedColumns(ReactTable);

class SheetExtendCompany extends Component {
	constructor(props) {
    super(props);
		...
		this.columns = [
			...
			{
				Header:'gl_account_id (recount account)',
				id: "gl_account_id",
				accessor: d => d.gl_account_id,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="gl_account_id" onChange={(event) => this.handleFilter(event)} value={this.state.options.gl_account_id} />
				)
            },
            {
				Header:'currency_id',
				...
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="currency_id" onChange={(event) => this.handleFilter(event)} value={this.state.options.currency_id} />
				)
            },
			{
				Header:'incoterm_id',
				id: "incoterm_id",
				accessor: d => d.incoterm_id,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="incoterm_id" onChange={(event) => this.handleFilter(event)} value={this.state.options.incoterm_id} />
				)
            },
			{
            ...
            },
            {
				...
				id: "terms_of_payment_id",
				accessor: d => d.terms_of_payment_id,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="terms_of_payment_id" onChange={(event) => this.handleFilter(event)} value={this.state.options.terms_of_payment_id} />
				)
            },
			{
				Header:'validasi',
				...
				Cell: (original) => (
					<React.Fragment>
						{original.value ? 
						<center>
							<Button id={'extend' + original.index} type="button" className="btn-xs btn-danger">
								show error
							</Button>
							<UncontrolledPopover
								trigger="legacy"  
								placement="left" 
								target={'extend' + original.index}
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
        ...
        this.props.fetchMigrasiExtendCompany(params)
        .then((resp) => {
                    if(this._isMounted){
...
const dispatchToProps = dispatch => {
	return {
		fetchMigrasiExtendCompany: (params) => dispatch(fetchMigrasiExtendCompany(params)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (SheetExtendCompany));