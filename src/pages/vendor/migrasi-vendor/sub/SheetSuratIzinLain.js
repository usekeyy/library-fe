...
import {fetchMigrasiSil} from '../../../../store/actions/vendor/migrasiVendorActions';
import {debounce} from '../../../../helpers/debounce';
const ReactTableFixedColumns = withFixedColumns(ReactTable);

class SheetSuratIzinLain extends Component {
	constructor(props) {
    super(props);
            ...
            // },
			{
				...
				Cell: (original) => (
					<React.Fragment>
						{original.value ? 
						<center>
							<Button id={'sil' + original.index} type="button" className="btn-xs btn-danger">
								show error
							</Button>
							<UncontrolledPopover
								trigger="legacy"  
								placement="left" 
								target={'sil' + original.index}
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
        ...
        this.props.fetchMigrasiSil(params)
        .then((resp) => {
                    if(this._isMounted){
...
const dispatchToProps = dispatch => {
	return {
		fetchMigrasiSil: (params) => dispatch(fetchMigrasiSil(params)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (SheetSuratIzinLain));