...
import {fetchMigrasiSiujk} from '../../../../store/actions/vendor/migrasiVendorActions';
import {debounce} from '../../../../helpers/debounce';
const ReactTableFixedColumns = withFixedColumns(ReactTable);

class SheetSiujk extends Component {
	constructor(props) {
    super(props);
		...
		this.columns = [
            ...
            // },
			// {
			// 	Header:'vendor_qualification_id',
			// 	id: "vendor_qualification_id",
			// 	accessor: d => d.vendor_qualification_id,
			// 	Filter: ({ filter, onChange }) => (
			// 		<input className="form-control" name="vendor_qualification_id" onChange={(event) => this.handleFilter(event)} value={this.state.options.vendor_qualification_id} />
			// 	)
            // },
            // {
			// 	Header:'status_siujk',
            ...
            // },
			{
				...
				Cell: (original) => (
					<React.Fragment>
						{original.value ? 
						<center>
							<Button id={'siujk' + original.index} type="button" className="btn-xs btn-danger">
								show error
							</Button>
							<UncontrolledPopover 
								trigger="legacy" 
								placement="left" 
								target={'siujk' + original.index}
						...
						:
						<center>
							<Button id={'siujk' + original.index} type="button" className="btn-xs btn-success">
								valid
							</Button>
						</center>}
					</React.Fragment>
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
        ...
        this.props.fetchMigrasiSiujk(this.state.uuid,params)
        .then((resp) => {
                    if(this._isMounted){
...
const dispatchToProps = dispatch => {
	return {
		fetchMigrasiSiujk: (params) => dispatch(fetchMigrasiSiujk(params)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (SheetSiujk));