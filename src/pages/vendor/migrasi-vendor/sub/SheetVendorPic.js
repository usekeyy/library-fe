...
import {fetchMigrasiVendorPic} from '../../../../store/actions/vendor/migrasiVendorActions';
...
class SheetVendorPic extends Component {
	constructor(props) {
    super(props);
		...
		this.columns = [
            ...
            },
			{
				...
				accessor: d => d.ktp_no,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="ktp_no" onChange={(event) => this.handleFilter(event)} value={this.state.options.ktp_no} />
				)
            },
            // {
			// 	Header:'jabatan',
			// 	id: "jabatan",
			// 	accessor: d => d.jabatan,
			// 	Filter: ({ filter, onChange }) => (
			// 		<input className="form-control" name="jabatan" onChange={(event) => this.handleFilter(event)} value={this.state.options.jabatan} />
			// 	)
            // },
			{
				Header:'phone_no',
				id: "phone_no",
				accessor: d => d.phone_no,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="phone_no" onChange={(event) => this.handleFilter(event)} value={this.state.options.phone_no} />
				)
            },
			{
				Header:'email',
            ...
            },
            {
				...
				accessor: d => d.main,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="main" onChange={(event) => this.handleFilter(event)} value={this.state.options.main} />
				)
            },
			{
				Header:'validasi',
				...
				Cell: (original) => (
					<React.Fragment>
						{original.value ? 
						<center>
							<Button id={'pic' + original.index} type="button" className="btn-xs btn-danger">
								show error
							</Button>
							<UncontrolledPopover
								trigger="legacy"  
								placement="left" 
								target={'pic' + original.index}
  ...
  componentDidMount = () => {
	...
	asyncData = async (params) => {
        ...
        this.props.fetchMigrasiVendorPic(params)
        .then((resp) => {
                    if(this._isMounted){
...
const dispatchToProps = dispatch => {
	return {
		fetchMigrasiVendorPic: (params) => dispatch(fetchMigrasiVendorPic(params)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (SheetVendorPic));