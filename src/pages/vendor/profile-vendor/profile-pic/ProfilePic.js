...
import FormProfilePic from './sub/FormProfilePic';
...
import { showPIC, showPICDetail, createPIC, deletePIC, updatePIC } from '../../../../store/actions/vendor/profile-vendor/picActions';
...
class ProfilePic extends React.Component {
  constructor(props) {
    ...
    this.state = {
		...
		activeTab: "1",
		activePill: "1",
		collapse: [{ id: 1, collapse: true, alias: "PIC" }],
		data: [],
    ...
    this.columns = [
      {
        Header: () => this.props.t("profileVendor:pic.nama"),
        accessor: "name",        
      },
      {
        Header: () => this.props.t("profileVendor:pic.nomor-ktp"),
        accessor: "ktp_no",        
      },
      {
        Header: () => this.props.t("profileVendor:pic.file-ktp"),
        accessor: "ktp_file",        
        Cell: ({original}) => (
					<React.Fragment>
      ...
      {
        Header: () => this.props.t("profileVendor:pic.jabatan"),
        accessor: "jabatan",        
      },
      {
        Header: () => this.props.t("profileVendor:pic.no-hp"),
        accessor: "phone_no",        
      },
      {
        Header: () => this.props.t("profileVendor:pic.email"),
        accessor: "email",        
	  },
	  {
		Header: () => this.props.t("profileVendor:pic.email-utama"),
		id: "main",
		accessor: d => this.statusEmailUtama(d.main),       
	  },
      {
        Header: "Status",
        id: "status",
        ...
        Cell: ({ value }) => (
          <React.Fragment>
            {this.props.getId.verification_uuid !== false && <center>
              {this.props.access.U && this.state.isVendor && !this.props.verification.has_draft_verification && <button disabled={this.state.verification.verifLength > 0} className="btn btn-xs btn-warning" onClick={(e) => this.showPayload(value.uuid)} ><i className="fa fa-edit"></i></button>}
              ...
              {this.props.access.A && !this.state.isVendor && (value.status !== 'y') && <button disabled={value.status === 'y' || this.state.verification.verifLength > 0} className="btn btn-xs btn-primary" onClick={(e) => this.saveVerificationLineItem(this.state.vendor_uuid, value.uuid, 'vendor_pic', { status: 'y' })}><span className={ (this.state.verification.loadingButton) ? "fas fa-spinner fa-pulse" : "fas fa-check"} disabled={this.state.verification.loadingButton}></span> </button>}
			  {this.props.access.A && !this.state.isVendor && (value.status !== 'y') && <button disabled={value.status === 'y' || this.state.verification.verifLength > 0} className="btn btn-xs btn-danger" onClick={(e) => this.saveVerificationLineItem(this.state.vendor_uuid, value.uuid, 'vendor_pic', { status: 'n' })}><span className={ (this.state.verification.loadingButton) ? "fas fa-spinner fa-pulse" : "fas fa-times"} disabled={this.state.verification.loadingButton}></span></button>}
            </center>}
          </React.Fragment>
        )
      },
  ...
  asyncData = async () => {
    ...
    this.props.showPIC(this.state.vendor_uuid, this.state.params)
      .then((resp) => {
        this.setState({
          data: resp.data.data,
  ...
  savePayload = (payload) => {
	...
	//   this.props.createPIC(this.state.vendor_uuid, payload)
		this.props.createPIC(payload)
        .then((resp) => {
          this.resetForm()
        ...
        .catch(error => {
          ...
          if (error !== undefined) {
            toastr.error(error.data.message,error?.data?.errors?.check_main_email_exist?.[0])
          } else {
  ...
  showPayload = (uuid) => {
    ...
    this.props.showPICDetail(uuid)
      .then((resp) => {
        var data = resp.data.data;
        this.setState({ uuid: data.uuid });
        var setSendData = { ...this.state }
        setSendData.sendData.pic_name = data.name;
        setSendData.sendData.pic_ktp_no = data.ktp_no;
        setSendData.sendData.pic_ktp_file = data.ktp_file;
        setSendData.sendData.pic_jabatan = data.jabatan;
        setSendData.sendData.pic_phone = data.phone_no;
        setSendData.sendData.pic_email = data.email;
		setSendData.sendData.status = data.status;
		setSendData.sendData.pic_email_utama = data.main === "y"? true : false;;
  ...
  updatePayload = (id, payload) => {
    if (this._isMounted) {
      ...
      dataPayload.vendor_id = this.state.vendor_id;
      this.props.updatePIC(id, dataPayload)
        .then((resp) => {
          this.resetForm();
          toastr.success(resp.data.message);
  ...
  toggleSweetAlert(name) {
    switch (name) {
      case 'confirm':
        this.deletePayload(this.state.delete_uuid)
        break;
      case 'cancel':
  ...
  resetForm = () => {
	var setSendData = { ...this.state }
	setSendData.sendData.pic_name = '';
	setSendData.sendData.pic_ktp_no = '';
	setSendData.sendData.pic_ktp_file = '';
	setSendData.sendData.pic_jabatan = '';
	setSendData.sendData.pic_phone = '';
	setSendData.sendData.pic_email = '';
	setSendData.sendData.status = '';
	setSendData.sendData.pic_email_utama = '';
  ...
  // VERIFICATION
	...
	statusEmailUtama = (data) => {
		switch (data) {
			case "y":
				return "Email PIC Utama"
			case "n":
				return
			default:
				return
  ...
  render() {
    ...
    const note_revisi = (this.props.verification.has_draft_verification === false && this.props.verification['PIC'].note !== null) ? (this.props.verification['PIC'].status !== null && this.props.verification['PIC'].status === 'y') ? undefined : true !== null : undefined;
    ...
    return (
      <div>
        <ol className="breadcrumb float-xl-right">
          ...
          <li className="breadcrumb-item active">PIC</li>
        </ol>
                    ...
                    <CardBody>
                      <div className="col-md-12">
                        {loading && <center><br/><ReactLoading type="cylon" color="#0f9e3e" /><br/></center>}
				                {!loading && this.state.isVendor && note_revisi !== undefined && <p className="text-danger"> <b> {t("profileVendor:catatan-revisi")} </b> {this.props.verification['PIC'].note} </p>}
...
const dispatchToProps = dispatch => {
  return {
    ...
    showPIC: (id,params) => dispatch(showPIC(id,params)),
	showPICDetail: (id) => dispatch(showPICDetail(id)),
	createPIC: (id) => dispatch(createPIC(id)),
	deletePIC: (uuid) => dispatch(deletePIC(uuid)),
	updatePIC: (uuid, payload) => dispatch(updatePIC(uuid,payload)),
...
export default connect(stateToProps, dispatchToProps)(withTranslation()(ProfilePic));
