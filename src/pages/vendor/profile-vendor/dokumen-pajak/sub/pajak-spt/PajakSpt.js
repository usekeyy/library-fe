...
import { fetchPajakSpt, savePajakSpt, updatePajakSpt, showPajakSpt, deletePajakSpt } from '../../../../../../store/actions/vendor/profile-vendor/pajakSptActions';
...
class PajakSpt extends Component {
	constructor(props) {
		super(props);
		...
		this.columns = [
			...
			{
				Header: () => this.props.t('profileVendor:spt-tahunan.nomor'),
				id: "number",
				accessor: d => d.number,
			},
			// {
			// 	Header: "Berlaku Sejak Tanggal",
			// 	id: "date",
			// 	accessor: d => d.date,
			// },
			{
				Header: () => this.props.t('profileVendor:spt-tahunan.tanggal'),
				id: "no",
				accessor: d => formatDate(d.date,false),
			},
			{
				Header: () => this.props.t('profileVendor:spt-tahunan.file'),
				id: "file",
				accessor: d => d.file,
				Cell: ({value}) => (
					<React.Fragment>
	...
	asyncData = async () => {
		...
		this.props.fetchPajakSpt(this.state.vendor_uuid, this.state.params)
			.then((resp) => {
				this.setState({
					data: resp.data.data,
...
toastr.error(resp.data.status, resp.data.message);
	...
	savePayload = (payload) => {
		if (this._isMounted) {
			...
			this.props.savePajakSpt(this.state.vendor_uuid, payload)
				.then((resp) => {
					this.resetForm()
	...
	showPayload = (vendor_id, uuid) => {
		...
		this.props.showPajakSpt(vendor_id, uuid)
			.then((resp) => {
	...
	updatePayload = (id, payload) => {
		if (this._isMounted) {
			this.setState({ loadingSubmit: true });
			this.props.updatePajakSpt(this.state.vendor_uuid, id, payload)
				.then((resp) => {
					this.resetForm();
					let load = { ...this.state.load };
					load.loadForm = false;
	...
	deletePayload = (vendor_uuid, uuid) => {
		this.props.deletePajakSpt(vendor_uuid, uuid)
			.then((resp) => {
				toastr.success(resp.data.message);
...
const dispatchToProps = dispatch => {
	return {
		...
		fetchPajakSpt: (id,params) => dispatch(fetchPajakSpt(id,params)),
		savePajakSpt: (vendor_uuid, params) => dispatch(savePajakSpt(vendor_uuid, params)),
		showPajakSpt: (vendor_uuid, params) => dispatch(showPajakSpt(vendor_uuid, params)),
		deletePajakSpt: (vendor_uuid, params) => dispatch(deletePajakSpt(vendor_uuid, params)),
		updatePajakSpt: (vendor_uuid, id, params) => dispatch(updatePajakSpt(vendor_uuid, id, params)),
...
export default connect(stateToProps, dispatchToProps)(withTranslation()(PajakSpt));