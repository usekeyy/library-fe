...
import TablePemegangSaham from './sub/TablePemegangSaham';
import { connect } from 'react-redux';
import { withTranslation } from 'react-i18next';
import {camelCase} from '../../../../helpers/camelCase';
import {statusKonfirmasiVerifikator} from '../../../../helpers/statusName';

class PemegangSaham extends React.Component {
	constructor(props) {
		super(props);
		...
		this.state = {
			...
			toggle : [],
			collapse : [
				{	id: 1, alias: 'Pemegang Saham', name: 'Pemegang Saham', type: 'pemegang-saham', collapse: true, },
			]
		};
	}
		...
		return (
			<div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item active">Pemegang Saham</li>
				</ol>
						...
						<div id="accordion" className="accordion">
							{
								...
								<Card className="bg-white text-black" key={i}>
									<CardHeader style={(this.state.isVendor && this.props.verification.has_draft_verification !== undefined && this.props.verification.has_draft_verification === false && this.props.verification[value.alias] !== undefined && this.props.verification[value.alias]).status==="n" ? colorPanelRed : colorPanelBlack } className={'card-header bg-dark-darker text-white set-pointer ' + (!value.collapse ? 'collapsed ' : '')} onClick={() => this.toggleCollapse(value.id)}>
										<i className="fa fa-circle f-s-8 mr-2 text-white-lighter"></i> {t("profileVendor:sub-accordion.pemegang-saham")}
										{value.type === 'pemegang-saham' &&
									...
									<Collapse isOpen={value.collapse}>
										<CardBody>
											<TablePemegangSaham title={value.name} revisi={(this.props.verification.has_draft_verification === false) ? this.props.verification[value.alias] : undefined} verification={this.props.verification} has_verification={this.props.verification} parentState={this.state} status_vendor={this.props.verification.status_vendor} collapse={() => this.toggleCollapse(value.id)} isInternal= {this.state.isInternal}/>
										</CardBody>
									</Collapse>
								</Card>
								))
...
const stateToProps = state => {
  ...
  export default connect(stateToProps, dispatchToProps)(withTranslation()(PemegangSaham));
