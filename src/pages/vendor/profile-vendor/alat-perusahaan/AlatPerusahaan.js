...
import FormAlatPerusahaan from "./sub/FormAlatPerusahaan";
...
import { fetchMesinPerusahaan, saveMesinPerusahaan, updateMesinPerusahaan, deleteMesinPerusahaan, showMesinPerusahaan } from '../../../../store/actions/vendor/profile-vendor/mesinPerusahaanActions'
...
class AlatPerusahaan extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      activeTab: "1",
      activePill: "1",
      collapse: [{ id: 1, collapse: true, alias: 'Alat' }],
      data: [],
    ...
    this.columns = [
      {
        Header: () => this.props.t("profileVendor:alat-mesin.nama"),
        accessor: "name",
      },
      {
        Header: () => this.props.t("profileVendor:alat-mesin.jumlah"),
        accessor: "qty",
      },
      {
        Header: () => this.props.t("profileVendor:alat-mesin.pengukuran"),
        accessor: "measurement",
      },
      {
        Header: () => this.props.t("profileVendor:alat-mesin.merk"),
        accessor: "merk",
      },
      {
        Header: () => this.props.t("profileVendor:alat-mesin.kondisi"),
        accessor: "condition",
      },
      {
        Header: () => this.props.t("profileVendor:alat-mesin.lokasi"),
        accessor: "location",
      },
      {
        Header: () => this.props.t("profileVendor:alat-mesin.kepemilikan"),
        accessor: "ownership",
      },
      {
        Header: "Status",
        ...
        Cell: ({ value }) => (
          <React.Fragment>
            {this.props.getId.verification_uuid !== false && <center>
              {this.props.access.U && !this.props.verification.has_draft_verification && this.state.isVendor && <button className="btn btn-xs btn-warning" disabled={this.state.verification.verifLength > 0} onClick={(e) => this.showPayload(this.state.vendor_uuid, value.uuid)}  ><i className="fa fa-edit"></i></button>}
              {this.props.access.U && !this.props.verification.has_draft_verification && this.state.isVendor && <button className="btn btn-xs btn-danger" disabled={this.state.verification.verifLength > 0} onClick={(e) => this.toggleConfirm(e, value.uuid)} ><i className="fa fa-trash"></i></button>}
              {this.props.access.A && !this.state.isVendor && (value.status !== 'y') && <button className="btn btn-xs btn-primary" disabled={value.status === 'y' || this.state.verification.verifLength > 0} onClick={(e) => this.saveVerificationLineItem(this.state.vendor_uuid, value.uuid, 'mesin_perusahaan', { status: 'y' })}><span className={ (this.state.verification.loadingButton) ? "fas fa-spinner fa-pulse" : "fas fa-check"} disabled={this.state.verification.loadingButton}></span> </button>}
							{this.props.access.A && !this.state.isVendor && (value.status !== 'y') && <button className="btn btn-xs btn-danger" disabled={value.status === 'y' || this.state.verification.verifLength > 0} onClick={(e) => this.saveVerificationLineItem(this.state.vendor_uuid, value.uuid, 'mesin_perusahaan', { status: 'n' })}><span className={ (this.state.verification.loadingButton) ? "fas fa-spinner fa-pulse" : "fas fa-times"} disabled={this.state.verification.loadingButton}></span></button>}
            </center>}
          </React.Fragment>
        )
      },
  ...
  asyncData = async () => {
    ...
    this.props.fetchMesinPerusahaan(this.state.vendor_uuid, this.state.params)
      .then((resp) => {
        this.setState({
          data: resp.data.data,
  ...
  savePayload = (payload) => {
    if (this._isMounted) {
      ...
      this.props.saveMesinPerusahaan(this.state.vendor_uuid, payload)
        .then((resp) => {
          this.resetForm()
  ...
  showPayload = (vendor_id, uuid) => {
    ...
    load.loadForm = true;
    this.setState({ load });
    this.props.showMesinPerusahaan(vendor_id, uuid)
      .then((resp) => {
        var data = resp.data.data;
        this.setState({ uuid: data.uuid });
        var setSendData = { ...this.state }
        setSendData.sendData.created_at = data.created_at;
        setSendData.sendData.condition = data.condition;
        setSendData.sendData.created_at = data.created_at;
        setSendData.sendData.created_by = data.created_by;
        setSendData.sendData.id = data.id;
        setSendData.sendData.information = data.information;
        setSendData.sendData.location = data.location;
        setSendData.sendData.measurement = data.measurement;
        setSendData.sendData.merk = data.merk;
        setSendData.sendData.name = data.name;
        setSendData.sendData.ownership = data.ownership;
        setSendData.sendData.qty = data.qty;
  ...
  updatePayload = (id, payload) => {
    if (this._isMounted) {
      ...
      this.props.updateMesinPerusahaan(this.state.vendor_uuid, id, payload)
        .then((resp) => {
          this.resetForm();
          let load = { ...this.state.load };
          load.loadForm = false;
  ...
  deletePayload = (vendor_uuid, uuid) => {
    this.props.deleteMesinPerusahaan(vendor_uuid, uuid)
      .then((resp) => {
        toastr.success(resp.data.message);
  ...
  resetForm = () => {
    ...
    setSendData.sendData.condition = '';
    setSendData.sendData.created_at = '';
    setSendData.sendData.created_by = '';
    setSendData.sendData.id = '';
    setSendData.sendData.information = '';
    setSendData.sendData.location = '';
    setSendData.sendData.measurement = '';
    setSendData.sendData.merk = '';
    setSendData.sendData.name = '';
    setSendData.sendData.ownership = '';
    setSendData.sendData.qty = '';
  ...
  render() {
    ...
    // const note_revisi = (this.props.verification.has_draft_verification === false && this.props.verification['Alat'].note !== null) ? true !== null : undefined;
    const note_revisi = (this.props.verification.has_draft_verification === false && this.props.verification['Alat'].note !== null) ? (this.props.verification['Alat'].status !== null && this.props.verification['Alat'].status === 'y') ? undefined : true !== null : undefined;
    ...
    return (
      <div>
        <ol className="breadcrumb float-xl-right">
          ...
          <li className="breadcrumb-item active">{t("profileVendor:sub-accordionalat-mesin-perusahaan")}</li>
        </ol>
            ...
            <div id="accordion" className="accordion">
              {this.state.collapse.map((value, i) => (
                <Card className="bg-white text-black" key={i}>
                  ...
                  >
                    <i className="fa fa-circle f-s-8 mr-2 text-white-lighter"></i> {t("profileVendor:sub-accordion.alat-mesin-perusahaan")}
                    ...
                    <CardBody>
                      <div className="col-md-12">
                        {loading && <center><br/><ReactLoading type="cylon" color="#0f9e3e" /><br/></center>}
												{!loading && this.state.isVendor && note_revisi !== undefined && <p className="text-danger"> <b> {t("profileVendor:catatan-revisi")} </b> {this.props.verification['Alat'].note} </p>}
...
const dispatchToProps = dispatch => {
  return {
    fetchMesinPerusahaan: (vendor_uuid, params) => dispatch(fetchMesinPerusahaan(vendor_uuid, params)),
    saveMesinPerusahaan: (vendor_uuid, params) => dispatch(saveMesinPerusahaan(vendor_uuid, params)),
    showMesinPerusahaan: (vendor_uuid, params) => dispatch(showMesinPerusahaan(vendor_uuid, params)),
    deleteMesinPerusahaan: (vendor_uuid, params) => dispatch(deleteMesinPerusahaan(vendor_uuid, params)),
    updateMesinPerusahaan: (vendor_uuid, id, params) => dispatch(updateMesinPerusahaan(vendor_uuid, id, params)),
...
export default connect(stateToProps, dispatchToProps)(withTranslation()(AlatPerusahaan));
