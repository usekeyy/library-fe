...
import { fetchKompetensi, saveKompetensi, updateKompetensi, showKompetensi, deleteKompetensi } from '../../../../store/actions/vendor/profile-vendor/kompetensiActions'
...
class Kompetensi extends React.Component {
  constructor(props) {
    ...
    this.columns = [
      ...
      {
        Header: () => this.props.t("profileVendor:kompetensi.tipe-rekanan"),
      ...
      {
        Header: () => this.props.t("profileVendor:kompetensi.lampiran"),
        accessor: "attachment",
        Cell: ({original}) => (
					<React.Fragment>
        ...
        Cell: ({ value }) => (
          <React.Fragment>
            {this.props.getId.verification_uuid !== false && <center>
              ...
              {this.props.access.A && !this.state.isVendor && (value.status !== 'y') && <button disabled={this.state.verification.verifLength > 0} className="btn btn-xs btn-primary" onClick={(e) => this.saveVerificationLineItem(this.state.vendor_uuid, value.uuid, 'kompetensi_vendor', { status: 'y' })}><span className={ (this.state.verification.loadingButton) ? "fas fa-spinner fa-pulse" : "fas fa-check"} disabled={this.state.verification.loadingButton}></span> </button>}
							{this.props.access.A && !this.state.isVendor && (value.status !== 'y') && <button disabled={this.state.verification.verifLength > 0} className="btn btn-xs btn-danger" onClick={(e) => this.saveVerificationLineItem(this.state.vendor_uuid, value.uuid, 'kompetensi_vendor', { status: 'n' })}><span className={ (this.state.verification.loadingButton) ? "fas fa-spinner fa-pulse" : "fas fa-times"} disabled={this.state.verification.loadingButton}></span></button>}
            </center>}
          </React.Fragment>
        )
      },
  ...
  asyncData = async () => {
    ...
    this.props.fetchKompetensi(this.state.vendor_uuid, this.state.params)
      .then((resp) => {
        let number = 0;
        const dataPlusNomor = resp.data.data.map((item,index) => {
          if (this.state.params.start === 10){
            number = index + 11
          }else{
            number++
          }
    ...
    this.props.showKompetensi(vendor_id, uuid)
      .then((resp) => {
        ...
        if(data.merk !== null && data.merk !== '' && data.merk !== undefined){
          ...
          if (data.bidang_usaha_id >= 1 && data.bidang_usaha_id <= 31){
  ...
  updatePayload = (id, payload) => {
    if (this._isMounted) {
      ...
      this.props.updateKompetensi(this.state.vendor_uuid, id, payload)
        .then((resp) => {
          this.resetForm();
          let load = { ...this.state.load };
          load.loadForm = false;
  ...
  deletePayload = (vendor_uuid, uuid) => {
    this.props.deleteKompetensi(vendor_uuid, uuid)
      .then((resp) => {
        toastr.success(resp.data.message);
        this.setState({ isConfirm: false , errors : {} });
        this.req()
      })
      .catch((error) => {
        if (error !== undefined) {
  ...
  saveItems = (payload) => {
    console.log(payload)
    let arr = this.state.merks;
    if(arr.length<10){
        arr.push(payload);
        this.setState({ merks: arr })
    }else{
        toastr.error('Merk yang diInput Max 10 Item')
    }   
  }
    ...
    return (
      <div>
        <ol className="breadcrumb float-xl-right">
          ...
          <li className="breadcrumb-item active">{t("profileVendor:sub-accordion.kompetensi")}</li>
        </ol>
            ...
            <div id="accordion" className="accordion">
              {this.state.collapse.map((value, i) => (
                <Card className="bg-white text-black" key={i}>
                  ...
                  >
                    <i className="fa fa-circle f-s-8 mr-2 text-white-lighter"></i> {t("profileVendor:sub-accordion.kompetensi")}
                  ...
                  <Collapse isOpen={value.collapse}>
                    <CardBody>
                      ...
                      {!loading && !this.state.isVendor && !this.state.verification.loadingNote && this.props.getId.verification_uuid !== false && <NoteVerificationData
...
const dispatchToProps = dispatch => {
  return {
    ...
    showKompetensi: (vendor_uuid, params) => dispatch(showKompetensi(vendor_uuid, params)),
    deleteKompetensi: (vendor_uuid, params) => dispatch(deleteKompetensi(vendor_uuid, params)),
    saveKompetensi: (vendor_uuid, params) => dispatch(saveKompetensi(vendor_uuid, params)),
    updateKompetensi: (vendor_uuid, id, params) => dispatch(updateKompetensi(vendor_uuid, id, params)),
