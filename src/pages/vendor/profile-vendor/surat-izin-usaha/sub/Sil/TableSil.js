...
class TableSil extends Component {
    constructor(props) {
        super(props);
        ...
        this.columns = [
            {
                Header: () => this.props.t("profileVendor:sil-number"),
                id: "number",
                accessor: d => d.number
            }, {
                Header: () => this.props.t("profileVendor:sil-name"),
                id: "name",
                accessor: d => d.name
            }, {
                Header: () => this.props.t("profileVendor:sil-file"),
                id: "file",
                accessor: d => d.file,
                Cell: ({original}) => (
					<React.Fragment>
                ...
                Header: () => this.props.t("profileVendor:sil-valid-from"),
                id: "start_date",
                accessor: d => formatDate(d.start_date, false)
            }, {
                Header: () => this.props.t("profileVendor:sil-valid-to"),
                id: "end_date",
                accessor: d => formatDate(d.end_date, false)
            }, {
                Header: () => this.props.t("profileVendor:sil-published-by"),
                id: "issued_by",
                ...
                Cell: ({value}) => (
                    <React.Fragment>
                        {this.props.verification_uuid !== false && <center>
                            {this.props.access.U && this.props.isVendor && !this.props.has_draft_verification && <button disabled={this.props.verification.verifLength > 0} className="btn btn-xs btn-warning" onClick={(e) => this.props.showSil(this.props.user_uuid, value.uuid)} ><span className="fa fa-edit"></span> </button>}
							...
							{this.props.access.A && !this.props.isVendor && (value.status !== 'y') && <button disabled={value.status === 'y' || this.props.verification.verifLength > 0} className="btn btn-xs btn-primary" onClick={(e) => this.props.saveVerificationItem(this.props.user_uuid, value.uuid, this.props.path, { status: 'y' })}><span className={ (this.props.sil.loadingButton) ? "fas fa-spinner fa-pulse" : "fas fa-check"} disabled={this.props.sil.loadingButton}></span> </button>}
							{this.props.access.A && !this.props.isVendor && (value.status !== 'y') && <button disabled={value.status === 'y' || this.props.verification.verifLength > 0} className="btn btn-xs btn-danger" onClick={(e) => this.props.saveVerificationItem(this.props.user_uuid, value.uuid, this.props.path, { status: 'n' })}><span className={ (this.props.sil.loadingButton) ? "fas fa-spinner fa-pulse" : "fas fa-times"} disabled={this.props.sil.loadingButton}></span></button>}
                        </center>}
                    </React.Fragment>
                )
            }
    ...
    asyncData = async (params) => {
        ...
        this
            .props
            .fetchSil(this.props.user_uuid, params)
            .then((resp) => {
                if (this._isMounted) {
...
export default withTranslation() (TableSil);