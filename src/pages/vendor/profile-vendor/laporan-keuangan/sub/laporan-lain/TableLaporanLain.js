...
class TableLaporanLain extends Component {
    constructor(props) {
        super(props);
        ...
        this.columns = [
            {
                ...
                Header: () => this.props.t("profileVendor:laporan-lain-jenis-laporan"),
                id: "jenis_laporan",
                accessor: d => d.jenis_laporan
            }, {
                ...
                Cell: ({value}) => (
                    <React.Fragment>
                        {this.props.verification_uuid !== false &&<center>
                            {this.props.access.U && this.props.isVendor && !this.props.has_draft_verification && <button disabled={this.props.verification.verifLength > 0} className="btn btn-xs btn-warning" onClick={(e) => this.props.showLaporanLain(this.props.user_uuid, value.uuid)} ><span className="fa fa-edit"></span> </button>}
							...
							{this.props.access.A && !this.props.isVendor && (value.status !== 'y') && <button disabled={value.status === 'y' || this.props.verification.verifLength > 0} className="btn btn-xs btn-primary" onClick={(e) => this.props.saveVerificationItem(this.props.user_uuid, value.uuid, this.props.path, { status: 'y' })}><span className={ (this.props.laporan_lain.loadingButton) ? "fas fa-spinner fa-pulse" : "fas fa-check"} disabled={this.props.laporan_lain.loadingButton}></span> </button>}
							{this.props.access.A && !this.props.isVendor && (value.status !== 'y') && <button disabled={value.status === 'y' || this.props.verification.verifLength > 0} className="btn btn-xs btn-danger" onClick={(e) => this.props.saveVerificationItem(this.props.user_uuid, value.uuid, this.props.path, { status: 'n' })}><span className={ (this.props.laporan_lain.loadingButton) ? "fas fa-spinner fa-pulse" : "fas fa-times"} disabled={this.props.laporan_lain.loadingButton}></span></button>}
                        </center>}
                    </React.Fragment>
                )
            }
    ...
    asyncData = async (params) => {
        ...
        this
            .props
            .fetchLaporanLain(this.props.user_uuid, params)
            .then((resp) => {
                if (this._isMounted) {
...
export default withTranslation() (TableLaporanLain);