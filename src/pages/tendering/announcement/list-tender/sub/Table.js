...
class Table extends Component {
    ...
    super(props);
		...
		this.state = {
			...
			loading: false,
			statusSearch: [
                { name: 'Registered', value: 'register', isChecked: false },
                { name: 'Quote', value: 'quote', isChecked: false },
				{ name: 'No Quote', value: 'no_quote', isChecked: false },
				{ name: 'Reject Quote', value: 'reject_quote', isChecked: false },
				{ name: 'Reject RFQ', value: 'reject_rfq', isChecked: false },
				{ name: 'Tidak Respon', value: 'tidak_respon', isChecked: false }
			]
		}
		...
		this.columns = [
			...
			},
			{
				...
				Cell: ({original}) => (
					<React.Fragment>
						<center>
							{<button className="btn btn-primary btn-xs" onClick={(e) => this.showDetail(e, original)} >View </button>}
						</center>
					</React.Fragment>
				)
			}
...
// }
	...
	asyncData = async (id, params) => {
      ...
      this.props.fetchUndanganTender(params)
      .then((resp) => {
				if(this._isMounted){
...
toastr.error(resp.data.status, resp.data.message);
	...
	getCheck = (check) => {
		...
		filters.start = 0;
		if(check.length > 0){
			filters.status_vendor = check.join(";");
			this.setState({options: filters}, () => this.fetchData())
		} else {
			filters.status_vendor = [];
			this.setState({options: filters}, () => this.fetchData())
		}
	}
