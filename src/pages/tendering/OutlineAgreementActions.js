...
export const fetchOutlineAgreement = (parameter) => {
    return dispatch => {
        return new Promise((resolve, reject) => {
            axios.get('tendering/outline_agreement', {
                params: parameter
            })
                .then(response => {
                    resolve(response);
...
export const releaseOA = (id, payload) => {
    return dispatch => {
        return new Promise((resolve, reject) => {
            axios.put('tendering/outline_agreement_release/'+id, payload)
                .then(response => {
                    resolve(response);
                    dispatch({
                        type: OutlineAgreementTypes.SUCCESS_FETCH_OUTLINE_AGREEMENT,
...
export const approvalOutlineAgreement = (id,payload) => {
    return dispatch => {
        return new Promise((resolve, reject) => {
            axios.post('tendering/outline_agreement/approve/'+id, payload)
                .then(response => {
                    resolve(response);
                    dispatch({
                        type: OutlineAgreementTypes.SUCCESS_APPROVE_OUTLINE_AGGREMENT,
                        data: response.data
                    })
                })
                .catch(error => {
                    reject(error.response);
                    dispatch({ type: OutlineAgreementTypes.FAIL_APPROVE_OUTLINE_AGGREMENT, response: error.response })
                });
        })
    }
}

export const downloadOa = (id) => {
    return dispatch => {
        return new Promise((resolve, reject) => {
            axios.get('/tendering/oa_generate_dokumen/'+id, {
                responseType : 'blob'
            })
                .then(response => {                    
                    resolve(response);
                    dispatch({
                        type: OutlineAgreementTypes.SUCCESS_DOWNLOAD_OUTLINE_AGGREMENT,
                        data: response.data
                    })
                })
                .catch(error => {
                    reject(error.response);
                    dispatch({ type: OutlineAgreementTypes.FAIL_DOWNLOAD_OUTLINE_AGGREMENT, response: error.response })
                });
        })
    }
}