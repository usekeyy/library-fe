		...
		this.columns = [
			...
			},
			{
				...
				Cell: ({value}) => (
					<React.Fragment>
						<center>
							{<button className={value.on_processing ? 'btn btn-primary btn-xs' : 'btn btn-success btn-xs'} onClick={(e) => this.showDetail(e, value.proposal_tender_uuid)} >{value.on_processing ? 'View' : 'Process'} </button>}
						</center>
					</React.Fragment>
				)
			}
