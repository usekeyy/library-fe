    ...
    super(props);
		...
		this.columns = [
			...
			},
			{
				...
				accessor: d => d.status,
				Filter: ({ filter, onChange }) => (
					<input className="form-control" name="status" onChange={(event) => this.handleFilter(event)} value={this.state.options.status} />
				)
			},
			{
				Header: () => 'Start Date',
				id: "start_date",
				accessor: d => formatDate(`${(d.start_date !== null) ? d.start_date : ''} ${(d.start_time !== null) ? d.start_time : ''}`, true),
				Filter: ({ filter, onChange }) => (
			...
			},
			{
				Header: () => 'End Date',
				id: "end_date",
				accessor: d => formatDate(`${(d.end_date !== null) ? d.end_date : ''} ${(d.end_time !== null) ? d.end_time : ''}`, true),
				Filter: ({ filter, onChange }) => (
			...
			},
			{
				...
				Cell: ({value}) => (
					<React.Fragment>
						<center>
							{(value.status === '' || value.status === 'rejected' || value.status === 'nego') && value.current === props.isRole && <button className="btn btn-success btn-xs" onClick={(e) => this.showDetail(e, value.uuid)} >Process</button>}
							{(value.status === 'nego' || value.status === 'rejected') && value.current !== this.props.isRole && <button className="btn btn-primary btn-xs" onClick={(e) => this.showDetail(e, value.uuid)} >View </button>}
						</center>
					</React.Fragment>
				)
			}
