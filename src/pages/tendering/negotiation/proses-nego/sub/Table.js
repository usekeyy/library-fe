    ...
    super(props);
		...
		this.columns = [
			...
			},
			{
				...
				filterable: false,
				sortable: false,
				accessor: d => (parseInt(d.tipe) === 1) ? "Nego Satuan" : "Nego Bersama",
			},
			{
				...
				Cell: ({value}) => (
					<React.Fragment>
						<center>
							{(value.status === '' || value.status === 'rejected' || value.status === 'nego') && value.current === this.props.isRole && <button className="btn btn-success btn-xs" onClick={(e) => this.showDetail(e, value.uuid)} >Process </button>}
							{/* {value.tipe === '2' && value.status === 'nego' && value.current !== this.props.isRole && <button className="btn btn-primary btn-xs" onClick={(e) => this.showDetail(e, value.uuid)} >Process </button>} */}
							{(value.status === 'nego' || value.status === 'rejected' || (value.tipe === "2" && value.status === "closed")) && value.current !== this.props.isRole && <button className="btn btn-primary btn-xs" onClick={(e) => this.showDetail(e, value.uuid)} >View </button>}
						</center>
					</React.Fragment>
				)
			}
