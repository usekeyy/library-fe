...
const Term = (props) => {
    	// const { t } = props;
		const {terms} = props.parentState.quotation;
		...
		console.log(terms)
            ...
            return (
                <tr key={i}>
									...
									<td>
										{<input type="text" className={validFileLampiran ? "form-control is-invalid" : "form-control"} name={`attachments[${i}].file`} ref={register({ required: false })} disabled={true} defaultValue={filelampiran[i] === '' ? vendor_doc_name : filelampiran[i]} />}
										...
										<center>
											{
												...
												// 		<input type="file" name={`terms[${i}].files`} ref={register({required: false})} onChange={(e) => changeFile(e, i)} disabled/>
