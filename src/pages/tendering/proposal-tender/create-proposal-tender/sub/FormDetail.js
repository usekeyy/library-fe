	...
	rows = props.data.account_assignment.map((dt, i) => {
		return (
			<tr key={i}>
				...
				<td>{formatNumber(dt.net_value,2)}</td>
				...
				<div className="form-row">
					...
					<div className="form-group col-md-6">
						...
						<Controller
							...
							defaultValue={((props.data.items.mrp_controller_id !== undefined && props.data.items.mrp_controller_id!=="" && props.data.items.mrp_controller_id!==null) ? {label:props.data.items.mrp_controller_id +" - "+ props.data.items.mrp_controllers_name, value:props.data.items.mrp_controller_id } : null)}
							isDisabled={props.disabledForm}
						/>
					</div>
