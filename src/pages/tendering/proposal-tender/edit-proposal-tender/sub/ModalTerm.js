...
const ModalTerm = (props) => {
	...
	const onSubmit = async data => {
		...
		if(errBterm === false && errBdetail === false && lengthTerms.length > 0){
			const headerDt = {
		...
		rows = data_item.map((dt, i) => {
				return (
						<tr key={i}>
								...
								<td>
									{modalType === 'teknikal' && <input type="number" readOnly={false} name={`terms[${i}].bobot`} ref={register({})} className={(errBobotTerm) ? "form-control is-invalid" : "form-control"}  defaultValue={(data_terms[i] && data_terms.length > 0 && data_terms !== undefined) ? data_terms[i].bobot : dt.bobot} disabled={header.metode_evaluasi.value === 'sistem_gugur'} />}
									{modalType === 'komersial' && <input type="checkbox" name={`terms[${i}].attachment`} defaultChecked={(data_terms[i]) ? data_terms[i].attachment : dt.attachment} ref={register({})} />}
								</td>
								<td>
		...
		rows_attachments = data_attach.map((dt, i) => {
				return (
						<tr key={i}>
								...
								<td>
									{modalType === 'teknikal' && <input type="checkbox" name={`terms_administratif[${i}].attachment`} defaultChecked={(header.terms_admin !== undefined && header.terms_admin[i]) ? header.terms_admin[i].attachment : dt.attachment} ref={register({})} />}
							...
							<div className="table-responsive m-t-10">
								<table className="table table-bordered table-striped table-sm text-nowrap">
									<thead>
										<tr>
											<th>Description</th>
											<th>{modalType === 'teknikal' ? 'Bobot' : 'Attachment'}</th>
											<th>Action</th>
										</tr>
									</thead>
									<tbody>{rows}</tbody>
