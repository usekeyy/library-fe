...
const Schedule = (props) => {
		...
		if (m_jadwal_tender.length > 0) {
			rows = m_jadwal_tender.map((dt, i) => {
				...
				console.log(schedules_value);
				const validStartDate = (i === 0) ? minDate : (startDatePicker[i] !== undefined) ? startDatePicker[i] : minDate;
				const start_date_value = schedules_value !== '' && schedules_value !== null && schedules_value?.length > 0 ? moment(schedules_value[0].start_date).format("YYYY-MM-DD") : '';
				const start_date_picker = schedules_value !== '' && schedules_value !== null && schedules_value?.length > 0 ? moment(schedules_value[0].start_date).format("DD-MM-YYYY") : '';
				const start_time_value = schedules_value !== '' && schedules_value?.length > 0 && schedules_value[0]?.start_time !== null ? schedules_value[0].start_time.split(":").slice(0, -1).join(":") : '';
				const end_date_value = schedules_value !== '' && schedules_value !== null && schedules_value?.length > 0 ? moment(schedules_value[0].end_date).format("YYYY-MM-DD") : '';
				const end_date_picker = schedules_value !== '' && schedules_value !== null && schedules_value?.length > 0 ? moment(schedules_value[0].end_date).format("DD-MM-YYYY") : '';
				const end_time_value = schedules_value !== '' && schedules_value[0]?.end_time !== null && schedules_value?.length > 0 ? schedules_value[0].end_time.split(":").slice(0, -1).join(":") : '';
				const datepickerValue = `${start_date_picker} ${start_time_value} - ${end_date_picker} ${end_time_value}`
						...
						return (
							...
							<td>
								{!show_metode_aanwijzing && showDatePicker[i] && <DateRangePicker 
									style={{ width: '300px' }}
								...
								>
									{!loadingDatePicker.includes(i) && <input type="text" style={{ width: '300px' }} readOnly={true} name={`schedule[${i}].daterangepicker`} ref={register({})} defaultValue={datepickerValue} className={errDatePicker[i] ? "form-control is-invalid" : "form-control"} />}
								</DateRangePicker>}
								<ul>
							...
							<td>
								...
								<input type="text" style={{ display: 'none' }} readOnly={true} name={`schedule[${i}].start_date`} ref={register({})} className={(errors[`schedule.${i}.start_date`]) ? "form-control is-invalid" : "form-control"} placeholder="" defaultValue={start_date_value} />
								<input type="text" style={{ display: 'none' }} readOnly={true} name={`schedule[${i}].start_time`} ref={register({})} className={(errors[`schedule.${i}.start_time`]) ? "form-control is-invalid" : "form-control"} placeholder="" defaultValue={start_time_value} />
								<input type="text" style={{ display: 'none' }} readOnly={true} name={`schedule[${i}].end_date`} ref={register({})} className={(errors[`schedule.${i}.end_date`]) ? "form-control is-invalid" : "form-control"} placeholder="" defaultValue={end_date_value} />
								<input type="text" style={{ display: 'none' }} readOnly={true} name={`schedule[${i}].end_time`} ref={register({})} className={(errors[`schedule.${i}.end_time`]) ? "form-control is-invalid" : "form-control"} placeholder="" defaultValue={end_time_value} />
							</td>
					</tr>
						)
				})
