...
const Attachment = (props) => {
    ...
    if (attachments.length > 0) {
            ...
            return (
                <tr key={i}>
                    ...
                    <td>
										...
										<td>
											{dt.type !== 'oe' && <input type="checkbox" name={`documents[${i}].share`} ref={register({})} defaultChecked={dt.share === "1" || dt.share === "y"} />}
											<input type="hidden" name={`documents[${i}].status`} ref={register({})} defaultValue={`d`} />
											<input type="hidden" name={`documents[${i}].purchasing_requisition_attachment_id`} ref={register({})} defaultValue={dt.purchasing_requisition_attachment_id} />
        ...
        rows_attactment = (<RowEmpty colSpan='9'>Tidak ada data</RowEmpty>);
		}
