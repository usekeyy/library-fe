                        ...
                        array_award.map(function (data_award, index_award) {
                            return (
                                <tr key={index_item + '/' + index_award}>
                                    ...
                                    {index_award === 0 && 
                                        <td rowSpan={total_awarding2} style={{width:"1%", textAlign: 'center', verticalAlign: 'middle'}}>
                                            <input disabled={props.user_uuid === props.created_by ? (props.status === 'd' || props.status === 'r' ? false : true) : true} type="checkbox" checked={data_item.is_retender === 'y' ? true : false} onChange={(e) => props.handleChecklistRetender(e, data_pr.pr_number+'-'+data_item.pr_item_number)} />
                                        </td>
                                    }
                                    {props.status === 'd' || props.status === 'r' ?
                                        <td style={{width:"1%", textAlign: 'center', verticalAlign: 'middle' }}>
                                            <input type="checkbox" checked={data_award.is_winner === 'y' ? true : false} 
                                                onChange={(e) => props.handleChecklistWinnerItem(e, data_pr.pr_number+'-'+data_item.pr_item_number+'-'+data_award.vendor_id)} 
                                                disabled={props.user_uuid === props.created_by ?
                                                    (data_item.is_retender === 'y' ? true : false) : true} />
                                        </td>
                                        : 
                                        (data_award.is_winner === 'y' ? 
...
export default withTranslation()(RekapNegoisasi);
