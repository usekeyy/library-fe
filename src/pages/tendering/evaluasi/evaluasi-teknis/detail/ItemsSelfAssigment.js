...
const ItemsSelfAssigment = (props) => {
    const { t } = props;  
    let rows;
            ...
            if (props.data[key]['tipe'] === "jasa") {
                ...
                return (
                    <tr key={(loop)}>
                        <td>{props.data[key]['number_pr']}</td>
                        <td>{props.data[key]['item_no']}</td>
                        <td>{(props.data[key]['material_no']==="" || props.data[key]['material_no']===undefined || props.data[key]['material_no']===null) ? "" : parseInt(props.data[key]['material_no'])}</td>
                        <td>{props.data[key]['short_text']}</td>
                        <td>{props.data[key]['spesifikasi']}</td>
                                ...
                                <table className="table table-bordered table-striped table-sm">
                                    <thead>
                                        <tr>
                                            ...
                                            <th align="center" rowSpan="2">{t("evaluation:label.short-text")}</th>
                                            <th align="center" rowSpan="2">{t("evaluation:label.specification")}</th>
                                            <th align="center" rowSpan="2">{t("evaluation:label.currency")}</th>
...
export default withTranslation()(ItemsSelfAssigment);
