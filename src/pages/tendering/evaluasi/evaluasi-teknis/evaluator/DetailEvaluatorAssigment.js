...
import Form from '../process/sub/Form';
import { Panel, PanelBody } from '../../../../../containers/layout/sub/panel/panel';
import { deleteEvaluasiTeknisAttactment, getEvaluasiTeknisCompare,showProcessEvaluasiTeknis,saveEvaluasiTeknisPublish, saveEvaluasiTeknisScore, storeEvaluasiTeknisKlarifikasi, getEvaluasiTeknisKlarifikasi, getEvaluasiTeknisAttactment, storeEvaluasiTeknisAttactment } from '../../../../../store/actions/tendering/evaluationTechnicalActions'
...
class DetailEvaluatorAssigment extends Component {
    constructor(props) {
        super(props)
    ...
    getKlarifikasi() {
        if (this._isMounted) {
            ...
            this.props.getEvaluasiTeknisKlarifikasi(this.props.match.params.id, this.props.match.params.uuid)
                .then((resp) => {
                    let datas = resp.data.data;
                    this.setState(({loadings})=> ({ 
    ...
    getEvaluasiTeknisCompare(pr_item_id, pr_service_id) {
        if (this._isMounted) {
            this.setState(({loadings})=> ({ 
                modalOpen:true,
                deviateData:[],
    ...
    back(e) {
        if(this.state.isMonitoring) { this.props.history.push('/tendering/monitoring-tender-buyer/detail_evaluasi/teknis_evaluator/'+this.props.match.params.id) } else { this.props.history.push('/tendering/evaluation-technical-assignment/evaluator/'+this.props.match.params.id) }
    }
    ...
    storeEvaluasiTeknisScore = (payload) => {
        ...
        this.props.saveEvaluasiTeknisScore(this.props.match.params.id, {
            ...
            evaluasi_id: this.state.data.evaluasi_id,
            ambang_batas: (this.state.data.ambang_batas===undefined || this.state.data.ambang_batas===null) ? 0:this.state.data.ambang_batas.ambang_batas,
            evaluasi: payload,
        })
            .then((resp) => {
                // let datas = resp.data.data;
                ...
                this.props.history.push('/tendering/evaluation-technical-assignment/evaluator/' + this.props.match.params.id)
            })
            .catch((resp) => {
                this.setState({ loading: false });
                toastr.error(resp.data.status, resp.data.message);
                ...
                {!this.state.loading &&
                    <div>
                        <Form
                            ...
                            getEvaluasiTeknisCompare={(pr_item_id, pr_service_id) => this.getEvaluasiTeknisCompare(pr_item_id, pr_service_id)}
...
const dispatchToProps = dispatch => {
    return {
        ...
        getEvaluasiTeknisKlarifikasi: (id, vendor_uuid) => dispatch(getEvaluasiTeknisKlarifikasi(id, vendor_uuid)),
        getEvaluasiTeknisAttactment: (id,vendor_id) => dispatch(getEvaluasiTeknisAttactment(id,vendor_id)),
...
export default connect(stateToProps, dispatchToProps)(withTranslation()(DetailEvaluatorAssigment))
