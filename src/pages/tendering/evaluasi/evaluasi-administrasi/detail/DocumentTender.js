        ...
        rows = Object.keys(props.data).map(function (key, index) {
            return (
                <tr key={key}>
                    ...
                    <td>{props.data[key]['file'] !=="" && <a className="pull-right" target="_blank" rel="noopener noreferrer" href={`${process.env.REACT_APP_API_BASE_URL}files/tendering/${props.data[key]['file']}` } > {props.data[key]['file']} </a>}</td>
                </tr>
            )
        });
    }
