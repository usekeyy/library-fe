...
import {fetchVendorQuotation, submitRegisterVendorQuotation} from '../../../store/actions/tendering/quotationActions';
	...
	handleAction = (type, data, uuid) => {
		switch (type) {
			case 'registrasi':	
				...
				break;
			case 'process':	
				this.props.history.push(`/task-vendor/quotation/detail/${uuid}`)
				break;
			case 'update':	
  ...
  render(){
    ...
    return (
      <div>
				...
				<Panel loading={false}>
					<PanelHeader>Quotation Table</PanelHeader>
					<PanelBody loading={false}>
						...
						<Table
							...
							fetchQuotation={this.props.fetchVendorQuotation}
...
const dispatchToProps = dispatch => {
	return {
		fetchVendorQuotation: (vendor_uuid, params) => dispatch(fetchVendorQuotation(vendor_uuid, params)),
		fileUpload: (id, payload) => dispatch(fileUpload(id, payload)),
		submitRegisterVendorQuotation: (id, payload) => dispatch(submitRegisterVendorQuotation(id, payload)),
	}
}

export default connect(stateToProps, dispatchToProps)( withTranslation() (Quotation));