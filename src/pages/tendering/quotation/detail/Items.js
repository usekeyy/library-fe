    ...
    // const { t } = props;
		...
		if (quoteItems.length > 0) {
				rows = quoteItems.map((dt, i) => {
					...
					const showGoods_options_quote_type = (header.metode_penyampaian_id==='2t' && header.current_step===2) ? true : false ;
					const showGoods_spec = (header.metode_penyampaian_id==='2t' && header.current_step===2) ? true : (!isServices && quotesValue === 'deviate' ? false : true);
					const showGoods_qty = (header.metode_penyampaian_id==='2t' && header.current_step===2) ? true :( !isServices && quotesValue === 'deviate' ? false : true);
					const showGoods_val = !isServices && (quotesValue === 'deviate' || quotesValue === 'comply') ? false : true;
					const showGoods_remark = (header.metode_penyampaian_id==='2t' && header.current_step===2) ? true : (!isServices && (quotesValue === 'no_quote' || quotesValue === '') ? true : false);
					const showGoods_dlv = (header.metode_penyampaian_id==='2t' && header.current_step===2) ? true : (quotesValue === 'no_quote' || quotesValue === '' ? true : false);
					...
					if(dt.service_lines && dt.service_lines.length > 0){
						service_lines = dt.service_lines.map((line, key) => {
							...
							return (
								...
								<tr key={line.id}>
									...
									<td>
										{!loadingInput.includes(line.uuid) &&
											<input type="number" min="0" step="0.01" onKeyPress={(e) => restrictNumber(e)} className={(errors[`service_lines.${index_services}.qty`]) ? "form-control is-invalid" : "form-control"} name={`service_lines[${line.id}].qty`} defaultValue={valueServices_qty} ref={register({})} disabled={showServices_qty}  />
										}
										{loadingInput.includes(line.uuid) && <i className="fas fa-spinner fa-pulse"></i>}
									...
									<td>{line.uom}</td>
									<td>
										<input type="number" min="0" step="0.01" onKeyPress={(e) => restrictNumber(e)} className={(errors[`service_lines.${index_services}.valuation_price`]) ? "form-control is-invalid" : "form-control"} name={`service_lines[${line.id}].valuation_price`} ref={register({})} defaultValue={valueServices_price} disabled={showServices_val} />
									...
									<td>{line.currency}</td>
									<td>
										<textarea rows="2" cols="5" className={(errors[`service_lines.${line.id}.remark`]) ? "form-control is-invalid" : "form-control"} name={`service_lines[${line.id}].remark`} ref={register({})} defaultValue={paramType === 'update' ? getServiceLines.length > 0 ? getServiceLines[0].remark : '' : line.remark} disabled={showServices_remark} />
									</td>
									<td>
										<input type="number" min="0" step="0.01" onKeyPress={(e) => restrictNumber(e)} className="form-control is-hidden width-auto" name={`service_lines[${line.id}].delivery_time`} ref={register({})} defaultValue={''} disabled={showServices_dlv} />
									</td>
								</tr>
								</React.Fragment>
							)
						...
						return (
								<React.Fragment key={i}>
									...
									<tr key={i}>
										<td>
											{loadingQuote === false && <select defaultValue={quotesValue} disabled={showGoods_options_quote_type} name={`items[${i}].quote`} onChange={(e) => handleChangeQuote(e, i, quoteItems[i], dt.service_lines, dt)} ref={register({})}>
						...
						<div className="row">
							...
							<div className="col-md-6">
								...
								<div className="form-group row">
										...
										<div className="col-sm-10">
												{/* <input type="number" readOnly={false} name={`header.bidbond`} ref={register({})} onBlur={(e) => handleChangeBiBond(e)} className={(errors['header.bidbond']) ? "form-control is-invalid" : "form-control"} placeholder="" defaultValue={tempData.bidbond ? tempData.bidbond : ''} /> */}
												<Controller
													as={NumberFormat} 
													control={control} 
													thousandSeparator={'.'} 
													decimalSeparator={','}
													name={`header.bidbond`}
													defaultValue={tempData.bidbond ? parseFloat(tempData.bidbond) : ''}
													onBlur={(e) => handleChangeBiBond(e)} 
													className={(errors['header.bidbond']) ? "form-control is-invalid" : "form-control"}
												/>
												{<span className="text-warning"> * Nilai Minimal Bid Bond {formatNumber((Total ? parseFloat(Total) : 0) * (parseFloat(header.bid_bond_value)/100), 2)} </span>}
								...
								<div className="form-group row">
										...
										<div className="col-sm-10">
												{/* <input type="date" readOnly={false} name={`header.bidbond_validity`} ref={register({})} className={(errors['header.bidbond_validity']) ? "form-control is-invalid" : "form-control"} placeholder="" defaultValue={tempData.bidbond_validity ? tempData.bidbond_validity : ''} /> */}
												<Controller
													control={control}
													name={`header.bidbond_validity`}
													defaultValue={tempData.bidbond_validity ? formattingDate(tempData.bidbond_validity) : ''}
													as={<Datetime
														value={tempData.bidbond_validity ? formattingDate(tempData.bidbond_validity) : ''}
														closeOnSelect={true}
														dateFormat="DD-MM-YYYY"
														timeFormat={false}
														inputProps={{ placeholder: "dd/mm/yyyy" }}
