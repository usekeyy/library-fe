    ...
    super(props);
		...
		this.columns = [
			...
			},
			{
				...
				id: "start_date",
				accessor: d => formatDate(`${d.start_date} ${d.start_time}`, true),
				Filter: ({ filter, onChange }) => (
			...
			},
			{
				...
				id: "end_date",
				accessor: d => formatDate(`${d.end_date} ${d.end_time}`, true),
				Filter: ({ filter, onChange }) => (
			...
			},
			{
				...
				Cell: ({value}) => (
					<React.Fragment>
						{value.metode_penyampaian_id !== '2t' && 
						<center>
							{
								...
								(new Date(`${value.end_date} ${value.end_time}`).getTime() > new Date().getTime()) && value.registration_quotation === 'submitted' && <button className="btn btn-primary btn-xs" onClick={(e) => this.props.handleAction('update', value, value.uuid)}>Update</button>
							}
						</center>}
						{value.metode_penyampaian_id === '2t' && 
						<center>
							{
								parseInt(value.current_step) === 1 && (value.registration_quotation === null || value.registration_quotation === 'open') && <button className="btn btn-primary btn-xs" onClick={(e) => this.props.handleAction('registrasi', value, value.uuid)}>Register</button>
							} {
								parseInt(value.current_step) === 1 && value.registration_quotation === 'register' && <button className="btn btn-primary btn-xs" onClick={(e) => this.props.handleAction('process', value, value.uuid)}>Process</button>
							} {
								parseInt(value.current_step) === 2 && value.registration_quotation === 'submitted' && <button className="btn btn-primary btn-xs" onClick={(e) => this.props.handleAction((parseInt(value.quotation_items) > 0) ? 'update' : 'process', value, value.uuid)}>Update</button>
							}
							{
								parseInt(value.current_step) === 1 && value.registration_quotation === 'submitted' && <button className="btn btn-primary btn-xs" onClick={(e) => this.props.handleAction('update', value, value.uuid)}>Update</button>
							}
						</center>}
					</React.Fragment>
				)
