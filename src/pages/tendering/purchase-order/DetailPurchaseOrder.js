...
import { fetchReminder, saveReminder, updateReminder, deleteReminder, showReminder } from '../../../store/actions/expediting/reminderActions';
...
// Form LOA
import FormLoa from './detail/loa/FormLoa';
...
export class DetailOpenPO extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            loadings: {
                ...
                payment_term:false,
                get_document_loa:true,
                loading_document_ckeditor : false,
            ...
            status_data_document_type: false,
    ...
    toggleClose = () => {
        this.setState(({ value_temp, status_detail_item }) => ({
            ...
            status_account_assignmment: false,
            isError: false, errors: [], tipe_modal: '',
        }));
	}
    ...
    getUUID = async () => {
        if (this._isMounted) {
            this.setState({ loading: true });
            this.props
                .showPurchaseOrder(this.props.match.params.id)
                .then((resp) => {
                    let datas = resp.data.data;
                    // datas.status = 'd'
                    ...
                    if (param_input.document_date === '' && datas.po_date !== null && datas.po_date !== '' && datas.po_date !== undefined) {
                        param_input.document_date = datas.po_date
                    }
                    ...
                    param_input.document_type_id = datas.purchase_order.tipe_po_id
                    param_input.document_type_description = datas.purchase_order.tipe_po
                    // param_input.document_type_param = []
                    // param_input.payment_term =[]
                    if (datas.purchase_order.tipe_po_id !== null && param_input.document_type_param.length === 0) {
                        param_input.document_type_id = datas.purchase_order.tipe_po_id
                        param_input.document_type_description = datas.purchase_order.tipe_po
                        param_input.document_type_param = {value: datas.purchase_order.tipe_po_id, label: datas.purchase_order.tipe_po_id + " - " + datas.purchase_order.tipe_po}
                    }
                    if(datas.purchase_order.terms_of_payment_id !== null && param_input.payment_term.length === 0){
                        param_input.payment_term = {value: datas.purchase_order.terms_of_payment_id, label: datas.purchase_order.terms_of_payment_id + " - " + datas.purchase_order.terms_of_payment_name}
                    }
    ...
    fetchDocumentType = (newValue) => {
        this.setState(({ loadings }) => ({
            loadings: { ...loadings, document_type: true },
        }));
        let select_params = (newValue !== '') ?
            {start: 0, length: 10, select: newValue, company_id: this.state.data.purc_org_id} :
            {start: 0, length: 10, company_id: this.state.data.purc_org_id};
        this.props.fetchDocumentType(select_params)
            .then((resp) => {
                // console.log(resp)
                let data = resp.data.data;
                let options = data.map((data) => {
                    let label = data.description_po !== null ? data.description_po : data.description
                    return { value: data.id, label: data.id + ' - ' + label};
                });
                this.setState(({ loadings }) => ({
                    loadings: { ...loadings, document_type: false },
                    m_document_type: options,
                }));
            })
            .catch((resp) => {
                this.setState(({ loadings }) => ({
    ...
    fetchReminder = (id) => {
        if (this._isMounted) {
            ...
            this.props.fetchReminder({po_id: id})
                .then((resp) => {
                    let datas = resp.data.data;
    ...
    fetchPaymentTerm = (newValue) => {
        this.setState(({ loadings }) => ({
            loadings: { ...loadings, payment_term: true },
        }));
        let select_params = (newValue !== '') ?
        ...
        this.props.fetchTermsOfPayment(select_params)
            .then((resp) => {
                ...
                this.setState(({ loadings }) => ({
                    loadings: { ...loadings, payment_term: false },
                    m_payment_term: options,
                }));
            })
            .catch((resp) => {
                this.setState(({ loadings }) => ({
        ...
        this.props.saveProgress(payload)
            .then((resp) => {
                this.setState(({ param_progress }) => ({
                    param_progress: { ...param_progress, uuid: '', id: '', number: '', planning: '', realitation: '', progress_date: '', description: '', attachment: [] }
                }), () => {
    ...
    updateProgress = async (payload) => {
        payload.uuid = this.state.param_progress.uuid
        payload.attachment = this.state.param_progress.attachment
        payload.po_id = this.state.data.id
        // console.log(payload)
        // return
        ...
        if (payload.file_name === '' || payload.file_name === null || payload.file_name === undefined) {
            ...
            this.props.saveDocumentPurchaseOrder(param)
                .then((resp) => {
                    toastr.success(resp.data.message);
                    this.fetchDocumentPurchaseOrder(this.state.data.uuid)
                    this.setState({ loading_document: false, modalDokumenPO: false, status_dokumen: false, status_dokumen_confirm: false })
                })
                .catch(error => {
                    if (error !== undefined) {
                        this.setState({ loading_document: false })
    ...
    savePurchaseOrder = (payload) => {
        this.setState({ status_data_item: false, status_header_text: false, status_due_date: false, status_data_document_type: false, status_data_incoterm_detail: false })
        let status = false
        ...
        if (param_input.document_type_id !== '' && (param_input.document_type_id === null || param_input.document_type_id === '')) {
            status = true
            this.setState({ status_data_document_type: true })
            return
        }
        ...
        this.setState({ status_dokumen: false })
        if (!this.state.data_document.some(d => d.tipe === '0' || d.tipe === 0) && (this.state.data.status_text !== 'Open PO' && this.state.data.status_text !== 'Rejected')) {
            console.log(this.state.data.status_text)
            this.setState({ status_dokumen: true })
            return
        }

        if (param_input.payment_term?.value !== "" && param_input.payment_term?.value!==undefined) {
            this.setState(({ param_input }) => ({ param_input: { ...param_input, payment_term_id : this.state.param_input?.payment_term?.value } }));
        }
        // console.log(payload)
    ...
    releasePurchaseOrder = (payload) => {
        let param = {
            status: payload.status,
            notes: payload.note,
            document_date : this.state.param_input.document_date,
            incoterm_detail : this.state.param_input.incoterm_detail,
            payment_term : this.state.param_input?.payment_term?.value
    ...
    deleteLampiranProgress = (key) => {
        ...
        // this.setState({ param_progress: new_data });
    }
    ...
    setOption = async (payload, type) => {
        ...
        switch(type) {
            case 'tipe-po':
                param_input.document_type_param = payload;
                param_input.document_type_id = payload.value;
                param_input.document_type_description = payload.label.split(' - ')[1]; //ambil nilai tipe_po
                break;
            case 'header-text':
            ...
            case 'due-date':
                ...
                break;
            case 'document-date':
                param_input.document_date = payload;
                break;
            case 'incoterm-id':
            ...
            case 'incoterm-detail':
                ...
                break;
            case 'payment-term':
                param_input.payment_term = payload;
                break;
            case 'notes':
    ...
    submitCreateLoa = (payload) => {
        this.setState(({ loadings }) => ({
            loadings: { ...loadings, loading_create_loa: true },
        }));
        this.props.createLOA(this.props.match.params.id, {
            'status' : payload.status,
    ...
    approvalLOA = (payload) => {
        ...
        this.props.approvalLOA(this.props.match.params.id, {
            'status' : payload.status,
            'deskripsi' : this.state.ckeditor
        })
        .then((resp) => {
            toastr.success(resp.data.message);
            this.setState(({ loadings, data }) => ({
        ...
        switch(param) {
          case 'item-detail':
            return <ItemDetail
                        ...
                        modalCreateLoa={this.modalCreateLoa}
          ...
          case 'modal-loa':
              return <FormLoa
                ...
                submitCreateLoa={(payload) => this.submitCreateLoa(payload)}
                approvalLOA={(payload) => this.approvalLOA(payload)}
                ...
                downloadGeneratePDFLOA={this.downloadGeneratePDFLOA}
              />
          default:
            return ;
        }
    ...
    downloadGeneratePDFLOA= () => {
		this.setState(({ loadings }) => ({ loadings: { ...loadings, loading_download_generate_loa: true } }));
		this.props.generatePDFLOA(this.props.match.params.id)
		.then((resp) => {
			const url = window.URL.createObjectURL(new Blob([resp.data]));
			const link = document.createElement('a');
			link.href = url;
			link.setAttribute('download', `LOA - ${this.state.data.eproc_number}.pdf`); //or any other extension
			document.body.appendChild(link);
			link.click();
    ...
    showDetailLoa= () => {
		this.setState(({ loadings }) => ({ loadings: { ...loadings, loading_download_generate_loa: true } }));
		this.props.showDetailLoa(this.props.match.params.id)
		.then((resp) => {
            let datas = resp.data.data
			console.log(datas)
			this.setState(
                ({ loadings }) => ({ loadings: { ...loadings, loading_download_generate_loa: false },
                data_po_loa : datas}
            ));
		})
		.catch((resp) => {
            ...
            <div>
                ...
                { !this.state.loading && 
                    <Form
                        ...
                        status_data_document_type={this.state.status_data_document_type}
                        ...
                        status_dokumen={this.state.status_dokumen}
                        fetchPaymentTerm={this.fetchPaymentTerm}
                        downloadGeneratePDFLOA={this.downloadGeneratePDFLOA}
                    />
                }
