...
const FormLoa = (props) => {
	...
	const onSubmit = async data => {
		...
		if(status==="d" || status==="s"){
			props.submitCreateLoa(data);
		}else{
			props.approvalLOA(data);
		}
		...
		<div>
			<form onSubmit={handleSubmit(onSubmit)}>
				...
				<div className="pull-right m-t-10 m-b-10">
                    ...
                    {props.data?.status!==null &&  <button className='btn btn-sm btn-success m-l-10' onClick={()=> props.downloadGeneratePDFLOA()} disabled={props.loadings.loading_download_generate_loa}>
...
export default  withTranslation() (FormLoa);