...
const FormDetail = (props) => {
	...
	const onSubmit = async data => {
		props.save( props.data.items.uuid, setData(data));
	};

	const setData = (data) => {
        data.valuation_price = (data.valuation_price === "") ? "" : parseFloat(data.valuation_price?.toString().replace(/\./g,'').replace(/,/g,'.'))
        return data;
    }
	...
	var total_price = isNaN(parseFloat(watchAllFields['valuation_price']?.toString().replace(/\./g,'').replace(/,/g,'.')))  ? (parseFloat(props.data.items.qty)* parseFloat(props.data.items.valuation_price)) : (parseFloat(props.data.items.qty)*parseFloat(watchAllFields['valuation_price']?.toString().replace(/\./g,'').replace(/,/g,'.')));
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				...
				<div className="form-row">
					<div className="form-group col-md-6">
						...
						<Controller
                            ...
                            control={control}
                            defaultValue={(props.data.items.valuation_price === "" || props.data.items.valuation_price === undefined) ? 0 : formatNumber(props.data.items.valuation_price,2)}
                            className="form-control"
                            as={<NumberFormat thousandSeparator={'.'} decimalSeparator={','} />}
                            disabled={ (props.isEditValuationPriceModal) ? false : props.disabledForm}
                        />
					</div>
				...
				<div className="form-row">
					...
					<div className="form-group col-md-6">
						...
						{props.isEditValuationPriceModal && <p>{formatNumber(total_price,2)}</p>}
						{!props.isEditValuationPriceModal &&
						<Controller
                            name="total_price"
                            control={control}
                            defaultValue={(props.data.items.total_price === "" || props.data.items.total_price === undefined) ? 0 : formatNumber(props.data.items.total_price,2)}
                            className="form-control"
                            as={<NumberFormat thousandSeparator={'.'} decimalSeparator={','} />}
                            disabled={props.disabledForm}
                        />
				...
				<div className="form-row">
					...
					<Panel>
						<div className="form-group">
							...
							<textarea disabled className="form-control" row="4" defaultValue={props.data.items.catatan_buyer}>
							</textarea>
						</div>
						<div className="form-group">
							<label>Disposisi Note</label>
							<textarea disabled className="form-control" row="4" defaultValue={props.data.items.disposisi_note}>
							</textarea>
						</div>
					</Panel>
				</ModalBody>
				<ModalFooter>
					...
					{props.isEditValuationPriceModal && <button className="btn btn-success" type="submit" disabled={props.loadings.loading_tombol_update_modal}>{props.loadings.loading_tombol_update_modal ? <i className="fa fa-spinner fa-spin"></i> : ''} {props.uuid !== "" ? t("currency:button.update") : t("currency:button.submit")}</button>}
				</ModalFooter>
			</form>
		</div>
	);
