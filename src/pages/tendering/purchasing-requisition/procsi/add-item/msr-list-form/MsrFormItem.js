...
const MsrFormItem = (props) => {
    ...
    const { t } = props;
    const onSubmit = data => {
			// const klasifikasi = data.clasifikasi?.value;
			// const service_params =  {
			// 		"seq": "1",
			// 		"distribution": "0",
			// 		"qty": data.qty,
            //         "requisitioner" : (data?.requisitioner?.value!==undefined) ? data?.requesitioner?.value: ""  ,
			// 		"net_value": data.valuation_price,
			// 		"gl_account_id": (data?.gl_accont?.value !== undefined) ? data?.gl_accont?.value : "00",
			// 		"cost_center_id": (data?.cost_center?.value !== undefined) ? data?.cost_center?.value : "B000000000",
			// 		"asset": (data?.asset_no?.value !== undefined) ? data?.asset_no?.value : "00",
			// 		"profit_center": "00",
			// 		"wbs_element": (data?.wbs_element?.value !== undefined) ? data?.wbs_element?.value : "00",
			// 		"line_number": "0000000010",
			// 		"activity_number": "0000000210",
			// 		"short_text": data.item_description,
			// 		"per": data.per,
			// 		"gross_value": "1100000",
			// 		"uom": data.uom?.value,
			// }
			// const services = (klasifikasi === 'jasa') ? [service_params] : []
        if (accAssgCategory === "U") {
            toastr.warning("Validation Account Assgment Category", "Please Choose Value Except U - Unknown");
        } else {
						const sendObj = {
							"item": {
								"tipe": data.clasifikasi?.value,
								"item_no": data.item_no?.value,
								"per": data.per,
								"qty": parseFloat(data.qty),
                                "requisitioner" : data.requisitioner?.value,
								"uom": data.uom?.value,
								"valuation_price": parseFloat(data.valuation_price),
								"total_value": parseFloat(data.total_value?.toString().replace(/\./g,'').replace(/,/g,'.')),
								"currency": data.currency?.value,
								"req_tracking_number": data.req_tracking_number,
								"requesitioner": data.requisitioner?.value,
								"plant_id": data.plant?.value,
								"mrp_controller_id": data.mrp?.value,
								"storage_location_id": data.storage_location?.value,
								"material_number": data.item_no?.value,
								"material_group_id": data.material_group?.value,
								"item_category": data.item_categori?.value,
								"short_text": data.item_description,
								"acc_assignment_category_id": data.ass_category?.value,
							...
							"account": [
								{
										...
										"asset": (data?.asset_no?.value !== undefined) ? data?.asset_no?.value : "00",
										"profit_center": "00",
										"wbs_element": (data?.wbs_element?.valu !== undefined) ? data?.wbs_element?.value : "00"
								}
							],
							"service": []
						};
            // console.log(sendObj)
            if((props.data.uuid !=="" && props.data.uuid!==undefined)){
                props.updateItemsPr(props.data.uuid,sendObj)
            }else{
                props.savePayload(sendObj);
            }
        }
    ...
    //         per: data.per,
    //         // eslint-disable-next-line no-useless-escape
    //         // valuation_price: parseFloat((data.valuation_price.length > 3) ? data.valuation_price.toString().replace(/\./g, '').replace(/\,/g, '.') : data.valuation_price.toString().replace(/\,/g, '.')),
    //         // eslint-disable-next-line no-useless-escape
	...
	// 					currency_id: (data.currency === undefined || data.currency === null) ? "" : data.currency?.value,
	// 					currency_name: (data.currency === undefined || data.currency === null) ? "" : data.currency?.label,
	// 					material_group_id: (data.material_group === undefined || data.material_group === null) ? "" : data.material_group?.value,
	// 					material_group_name: (data.material_group === undefined || data.material_group === null) ? "" : data.material_group?.label,
    //     }
    ...
    const handleChangeMaterialGroup = (selected) => {
        props.getMaterialGroup(selected)
    }
    ...
    const handleWorkUnits = (selected) => {
        props.getWorkUnits(selected)
    }
    ...
    const customStyles = {
		...
		const show = false;
		// (qty/per)*valuation price
		const total_value = (watchAllFields['qty']/watchAllFields['per'])*watchAllFields['valuation_price'];
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <ModalBody>
                    ...
                    <div className="form-group">
                        ...
                        {props.errors.item_categori && <span className="text-danger">{props.errors.item_categori[0]}</span>}
                    </div>

                    <div className="form-group">
                        <label >Material Number <span className="text-danger">*</span> </label>
                        ...
                        {props.errors.item_no && <span className="text-danger">{props.errors.item_no[0]}</span>}
                    </div>
                    ...
                    <div className="form-group">
                        ...
                        {props.errors.item_description && <span className="text-danger">{props.errors.item_description[0]}</span>}
                    </div>
                    ...
                    <div className="form-group">
                        ...
                        {props.errors.qty && <span className="text-danger">{props.errors.qty[0]}</span>}
                    </div>
                    <div className="form-group">
                        <label >Requisitioner <span className="text-danger">*</span> </label>
                        <Controller
                            components={animatedComponents}
                            closeMenuOnSelect={true}
                            as={Select}
                            // styles={props.errors.type ? customStyles : {}}
                            defaultValue={props.data.requisitioner}
                            control={control}
                            options={props.work_units}
                            onInputChange={handleWorkUnits}
                            name="requisitioner"
                            isClearable
                            isLoading={props.isLoading.work_units}
                            isDisabled={(props.disableModal === true) ? true : false}
                            rules={{ required: true }}
                            styles={errors.requisitioner ? customStyles : {}}
                        />
                        {/* <input type="text" defaultValue={(props.data.requisitioner === "" || props.data.requisitioner === undefined) ? 0 : props.data.requisitioner} disabled={false} name="requisitioner" ref={register({})} className="form-control" placeholder="" /> */}
                        {errors.requisitioner && <span className="text-danger">This field is required</span>}
                        {props.errors.requisitioner && <span className="text-danger">{props.errors.requisitioner[0]}</span>}
                    </div>
                    ...
                    <div className="form-group">
                        <label >Per</label>
                        <input type="number" disabled={(props.disableModal === true) ? true : false} name="per" ref={register({})} className="form-control" placeholder="" defaultValue={(props.data.per==="" ||props.data.per===undefined) ? 1 : props.data.per } />
                        {props.errors.per && <span className="text-danger">{props.errors.per[0]}</span>}
                    </div>
                    <div className="form-group">
                        ...
                        <Controller
                            ...
                            control={control}
                            defaultValue={(props.data.valuation_price === "" || props.data.valuation_price === undefined) ? 0 : props.data.valuation_price}
                            className="form-control"
                            as={<NumberFormat  decimalSeparator={'.'} decimalScale={2}/>}
                            disabled={(props.disableModal === true) ? true : false}
                            rules={{
                        ...
                        {props.errors.valuation_price && <span className="text-danger">{props.errors.valuation_price[0]}</span>}
                    </div>
                    {show && <div className="form-group">
                        ...
                        />
                        {/* <input type="number" defaultValue={props.data.valuation_price} disabled={(props.disableModal==true) ? true:false} name="valuation_price" ref={register({})} className="form-control" placeholder="" /> */}
                    ...
                    <div className="form-group">
                        ...
                        <input type="text" name="total_value" value={formatNumber(total_value,2)} disabled={true} ref={register({})} className="form-control" placeholder="" />
                        {errors.total_value && <span className="text-danger">{errors.total_value.type === "required" ? "Field harus diisi" : ''}  {errors.total_value.message}</span>}
                    </div>
										{/* <div className="form-group">
                    ...
                    <div className="form-group">
                        ...
                        {props.errors.plant_id && <span className="text-danger">{props.errors.plant_id[0]}</span>}
                    </div>
                    <div className="form-group">
                        <label >Storage Location  <span className="text-danger">*</span></label>
                        <Controller
                            components={animatedComponents}
                            closeMenuOnSelect={true}
                            as={Select}
                        ...
                        {props.errors.item?.storage_location_id && <span className="text-danger">{props.errors.item?.storage_location_id[0]}</span>}
                    </div>
                    <div className="form-group">
                        <label >MRP</label>
                        <Controller
                        ...
                        {props.errors.mrp_controller_id && <span className="text-danger">{props.errors.mrp_controller_id[0]}</span>}
                    </div>
                    <div className="form-group">
                        ...
                        <Controller
                            ...
                            onInputChange={handleChangeCurrency}
                            isDisabled={(props.disableModal === true) ? true : false}
                            rules={{ required: true }}
                        />
                        {errors.currency && <span className="text-danger">This field is required</span>}
                        {props.errors.item?.currency_id && <span className="text-danger">{props.errors.item?.currency_id[0]}</span>}
                    </div>
                    <div className="form-group">
                        ...
                        <Controller
                            ...
                            defaultValue={props.data.material_group}
                            onInputChange={handleChangeMaterialGroup}
                            isDisabled={(props.disableModal === true) ? true : false}
                            rules={{ required: true }}
                        />
                        {errors.material_group && <span className="text-danger">This field is required</span>}
                        {props.errors.item?.material_group_id && <span className="text-danger">{props.errors.item?.material_group_id[0]}</span>}
                    </div>
										<div className="form-group">
                        <label >Req Tracking Number</label>
                        <input type="text" defaultValue={props.data.req_tracking_number} disabled={(props.disableModal === true) ? true : false} name="req_tracking_number" ref={register({})} className="form-control" placeholder="" />
                        {props.errors.req_tracking_number && <span className="text-danger">{props.errors.req_tracking_number[0]}</span>}
                    </div>
                    {/* <div className="form-group">
                    ...
                    <div className="form-group">
                        ...
                        <Controller
                            ...
                            name="ass_category"
                            defaultValue={props.data.acc_assignment_category_id}
                        ...
                        {props.errors.account_category_id && <span className="text-danger">{props.errors.account_category_id[0]}</span>}
                    </div>
                    ...
                    {accAssgCategory === "A" &&
                        ...
                        <div className="form-group">
                            ...
                            {props.errors.asset_no && <span className="text-danger">{props.errors.asset_no[0]}</span>}
                        </div>
                    ...
                    {/* <div className="form-group">
                        ...
                        {errors.year && <span className="text-danger">This field is required</span>}
										...
										disabled={false}
										defaultValue={(props.data.short_text === "" || props.data.short_text === undefined) ? "" : props.data.short_text}
										/>
									</Panel> */}
                        <Panel >
                            <PanelHeader noButton={true}>
                ...
                {props.disableModal === false &&
                    <ModalFooter>
                        ...
                        <button className="btn btn-success" type="submit" disabled={props.loadings.loading_submit_modal}>{props.loadings.loading_submit_modal ? <i className="fa fa-spinner fa-spin"></i> : ''} {(props.data.uuid !=="" && props.data.uuid!==undefined )? "Update" : "Submit"} </button>
                    </ModalFooter>
                }
            </form>
        </div>
