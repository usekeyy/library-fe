...
class ModalForm extends Component {
  ...
  fetchWorkUnits = (params) => {
    if (this._isMounted) {
      if (params !== undefined) {
        this.setState(({ loadings, work_units }) => ({
          loadings: { ...loadings, work_units: true },
          work_units: []
        }));
      }
      let select_params =
      ...
      this.props
        ...
        .then((resp) => {
          ...
          this.setState(({ loadings, work_units }) => ({
            loadings: { ...loadings, work_units: false },
            work_units: options,
          }));
        })
        .catch((resp) => {
    ...
    if (newValue === undefined || newValue !== "") {
      ...
      this.props
        ...
        .then((resp) => {
          let data = resp.data.data;
          let options = data.map((data) => {
            return { value: data.id, label: data.short_text };
          });
    ...
    if (newValue === undefined || newValue !== "") {
      ...
      this.props
        ...
        .then((resp) => {
          ...
          this.setState(({ loadings, material_group }) => ({
            loadings: { ...loadings, material_group: false },
            material_group: options,
          }));
        })
        .catch((resp) => {
          this.setState(({ loadings }) => ({
      ...
      <div>
        ...
        <ModalHeader toggle={() => this.toggleClose()}>Modal {}</ModalHeader>
          <MsrFormItem
            ...
            optionsMaterialGroup={this.state.material_group}
            work_units={this.state.work_units}
            toggleClose={this.toggleClose}
            getWorkUnits = {(payload) => this.fetchWorkUnits(payload)}
            ...
            updateItemsPr={(id, payload) => this.props.updateItemsPr(id, payload)}
            saveitems={(payload) => this.props.saveitems(payload)}
            showItemNumberId={(payload) => this.showItemNumberId(payload)}
            savePayload={(payload) => this.props.savePayload(payload)}
