				...
				Cell: ({ value }) => (
					<React.Fragment>
						<center>
							{value.source === 'procsi' && 
								<React.Fragment>
									{value.status_pr_manual === 'd' && <Button color="lime" size="xs" value={value.uuid} onClick={(e) => this.toAddItem(e, value.uuid)} >Add</Button>}
									{value.status_pr_manual === 's' && <Button color="lime" size="xs" value={value.uuid} onClick={(e) => this.toAddItem(e, value.uuid)} >Detail</Button>}
									{value.status_pr_manual !== 'd' && value.status_pr_manual !== 's' && <Button color="primary" size="xs" value={value.uuid} onClick={(e) => this.toDetail(e, value.uuid)} >Process </Button>}
								</React.Fragment>
							}
							{value.source !== 'procsi' && <Button color="primary" size="xs" value={value.uuid} onClick={(e) => this.toDetail(e, value.uuid)} >Process </Button>}
...
handleFilter = (event) => {
	...
	toAddItem = (e, id) => {
		// e.preventDefault();
		this.props.parentProps.history.push(`/tendering/purchasing-requisition-procsi/create-item/${id}`);
	}
