...
const ModalEdoc = (props) => {
	...
	const [title] = React.useState(props.parentState.tempData.edocs.title)
	...
	const setData = (data) => {
		...
		data.created_by = props.parentProps.user.uuid;
		data.created_by_name = props.parentProps.user.name;
		data.edoc = {
			title: data.title,
								...
								<td>
									<div className="form-group">
										<div>
											...
											<input type="text" name={`items[${i}].title`} ref={register({})} defaultValue={(edocuments[i].title) ? edocuments[i].title : ''} placeholder="type a content title ..." className="form-control" />
